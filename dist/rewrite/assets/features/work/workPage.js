...
export function renderWorkPage(root, identity, navigation) {
    ...
    page.innerHTML = `<section class="page-heading work-toolbar"><div><p class="eyebrow">Daily operations</p><h1>Work</h1><p>Control today first, then open earlier workdays when their full operational evidence is needed.</p><small id="workDateLabel" class="muted"></small></div><div class="heading-actions toolbar-actions"><button class="button secondary compact" id="workPrev">←</button><button class="button secondary" id="workToday">Today</button><button class="button secondary compact" id="workNext">→</button><button class="button secondary" id="workRefresh">Refresh</button></div></section><div id="workError" class="error-box hidden"></div><section id="workSummary"></section><section id="workAttention"></section><section id="workTeams" class="work-team-grid"><div class="loading-state">Loading work…</div></section>`;
