...
export function openVisitActionDialog(visit, account, teams, hold, callbacks) {
    ...
    let pending = '';
    const showPanel = (panel) => { sections.forEach(s => s.classList.add('hidden')); for (const p of [reasonPanel, reschedulePanel, dnsPanel, holdPanel])
        p.classList.add('hidden'); panel.classList.remove('hidden'); };
    const showSections = () => { for (const p of [reasonPanel, reschedulePanel, dnsPanel, holdPanel])
