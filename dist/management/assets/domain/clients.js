export function normaliseAccount(input, businessId) {
...
export function normaliseLocation(input, businessId) {
    if (!input.accountId)
        throw new Error('A service location must belong to an account.');
...
export function normaliseAgreement(input, businessId) {
    if (!input.accountId || !input.serviceLocationId)
        throw new Error('Agreement account and service location are required.');
    if (!input.defaultTeamId)
        throw new Error('Default team is required.');
    if (!input.startDate)
    ...
    if (!weekdays.length)
        throw new Error('Choose at least one service day.');
    if (input.frequency !== 'weekly' && weekdays.length > 1)
        throw new Error('Fortnightly, four-weekly and monthly agreements use one recurring day.');
    if (input.frequency === 'monthly' && (input.monthlyOrdinal ?? 0) < 1)
        throw new Error('Monthly agreements require a week-of-month.');
...
export function locationsForAccount(workspace, accountId) { return workspace.locations.filter(row => row.accountId === accountId).sort((a, b) => (a.siteName || a.address).localeCompare(b.siteName || b.address)); }
export function agreementsForAccount(workspace, accountId) { return workspace.agreements.filter(row => row.accountId === accountId).sort((a, b) => (a.startDate || '').localeCompare(b.startDate || '')); }
export function agreementsForLocation(workspace, locationId) { return workspace.agreements.filter(row => row.serviceLocationId === locationId).sort((a, b) => (b.startDate || '').localeCompare(a.startDate || '')); }
export function agreementFrequencyLabel(agreement) {
    if (agreement.frequency === 'weekly' && agreement.weekdays.length > 1)
        return `${agreement.weekdays.length}× weekly`;
