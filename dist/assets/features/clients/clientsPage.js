...
export function renderClientsPage(root, identity, navigation) {
    ...
    page.innerHTML = `<section class="clients-toolbar"><div><p class="eyebrow">Clients</p><h1>Accounts & service locations</h1><p class="muted">Keep the billing account separate from every physical property you service.</p></div><div class="toolbar-actions"><button id="refreshClients">Refresh</button><button id="newAccount" class="primary-button">+ New account</button></div></section><div id="clientError" class="error-box hidden" role="alert"></div><section class="clients-workspace"><aside class="client-list-panel"><div class="client-search"><input id="clientSearch" type="search" placeholder="Search client, address or suburb"><span id="clientCount"></span></div><div id="clientList" class="client-list"><div class="loading-state">Loading clients…</div></div></aside><section id="clientDetail" class="client-detail"><div class="empty-detail"><strong>Select a client</strong><span>Account, service locations and routine agreements will appear here.</span></div></section></section>`;
