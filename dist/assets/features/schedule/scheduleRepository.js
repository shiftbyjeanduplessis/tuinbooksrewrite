...
export async function loadWeek(businessId, weekStart) {
    ...
    const services = (Array.isArray(raw.services) ? raw.services : []).map((r) => ({ id: String(r.id), businessId, name: String(r.name ?? r.id ?? 'Service'), notes: String(r.notes ?? ''), active: r.active !== false }));
    const agreements = (Array.isArray(raw.agreements) ? raw.agreements : []).map((r) => ({ id: String(r.id), businessId, accountId: String(r.client_id), serviceLocationId: String(r.service_site_id), status: String(r.status ?? 'draft'), startDate: r.start_date ? String(r.start_date) : null, endDate: r.end_date ? String(r.end_date) : null, frequency: String(r.frequency ?? 'weekly'), weekdays: Array.isArray(r.weekdays) ? r.weekdays.map(Number) : [], monthlyOrdinal: r.monthly_ordinal == null ? null : Number(r.monthly_ordinal), defaultTeamId: String(r.default_team_id ?? ''), estimatedMinutes: Number(r.estimated_minutes ?? 60), serviceIds: Array.isArray(r.service_ids) ? r.service_ids.map(String) : [], notes: String(r.notes ?? ''), monthlyFee: r.monthly_fee == null ? null : Number(r.monthly_fee), scheduleSeriesId: r.schedule_series_id ? String(r.schedule_series_id) : null, version: Number(r.version ?? 0), updatedAt: r.updated_at ? String(r.updated_at) : null }));
    ...
    return { weekStart, weekEnd, teams, accounts, locations, services, agreements, visits, queueItems, series, clientHolds, dayActions };
}
...
export async function persistResolveMissed(businessId, visitId, decision, note) { await rpc('tuinbooks_v2_resolve_missed_visit_r12', { p_business_id: businessId, p_visit_id: visitId, p_decision: decision, p_work_record_id: `work-r12-${crypto.randomUUID()}`, p_note: note }); }
