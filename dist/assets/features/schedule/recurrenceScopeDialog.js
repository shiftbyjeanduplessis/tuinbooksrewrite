...
export function chooseMoveScope(visit, series) {
    return new Promise(resolve => {
        ...
        let settled = false;
        const done = (value) => { if (settled)
            return; settled = true; resolve(value); dialog.close(); };
        dialog.querySelector('[data-scope="one"]').onclick = () => done('one');
        dialog.querySelector('[data-scope="future"]').onclick = () => done('future');
        dialog.addEventListener('cancel', event => { event.preventDefault(); done(null); });
        dialog.addEventListener('close', () => { if (!settled)
            resolve(null); dialog.remove(); }, { once: true });
        dialog.showModal();
    });
}
function esc(v) { return v.replace(/[&<>'"]/g, ch => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', "'": '&#39;', '"': '&quot;' }[ch] ?? ch)); }
//# sourceMappingURL=recurrenceScopeDialog.js.map