(() => {
  ...
  const BUILD = '60.5.8-cloud-reconciled-rolling-readable-cards';
  if (window.__tuinbooksVisitControlsV6058 === BUILD) return;
  window.__tuinbooksVisitControlsV6058 = BUILD;
  ...
  /* -----------------------------------------------------------------------
     ...
     v60.5.8 never writes a recurring occurrence until it has first read the
  ...
  function movedOccurrenceExistsV6058(clientId,weekStart){
  ...
  function rollingWorkspaceReadyV6058(){
    ...
    if(managementRouteV6058())return !!b.managementCoreReadyV5950&&!!(b.managementOperationalReadyV5950||b.managementOperationalReadyV59371)&&!!b.lastOperationalJson;
    if(String(b.mode||'').toLowerCase()==='supabase')return !!b.businessId&&!b.operationalHydrating&&!!b.lastOperationalJson;
    return true;
  }
  function cloudRoutineRowV6058(row){
    const p=row?.payload||{};
  ...
  async function cloudScheduleHorizonV6058(firstWeek,horizonEnd){
    ...
    const {data,error}=await b.client.from('schedule_jobs')
      ...
      .eq('business_id',b.businessId).gte('visit_date',firstWeek).lte('visit_date',horizonEnd).order('visit_date');
    if(error)throw error;
    return Array.isArray(data)?data:[];
  }
  function cloudOccurrenceIndexV6058(rows){
    const keys=new Set(),weeks=new Set(),moved=new Set();
    (rows||[]).forEach(row=>{
      if(!cloudRoutineRowV6058(row))return;
  ...
  function recurrenceCollisionV6058(error){
  ...
  function operationalDeltaRpcV6058(){
  ...
  async function saveRollingCandidatesV6058(candidates){
    const b=window.backendV28||{};
    ...
    const rpc=operationalDeltaRpcV6058();
    const payload={p_business_id:b.businessId,p_expected_revision:Number(b.operationalRevision||0),p_schedules:candidates.map(scheduleCloudRowV6058),p_work_records:[],p_opportunities:[],p_quotes:[],p_invoices:[],p_meta:null,p_deleted_schedule_ids:[],p_deleted_quote_ids:[],p_deleted_invoice_ids:[]};
  ...
  function adoptSavedCandidatesV6058(candidates,revision){
  ...
  function clearStaleRecurrenceErrorV6058(){
    const b=window.backendV28||{},e=b.lastOperationalErrorV5604||{};
    if(!recurrenceCollisionV6058(e))return;
  ...
  async function maintainRollingScheduleV6058(reason='automatic'){
    if(rollingBusy)return {busy:true};
    if(!rollingWorkspaceReadyV6058())return {ready:false};
    rollingBusy=true;
    try{
      clearStaleRecurrenceErrorV6058();
      ...
      let cloudRows=await cloudScheduleHorizonV6058(firstWeek,horizonEnd),cloudIndex=cloudOccurrenceIndexV6058(cloudRows);
      ...
      for(const client of (stateNow().clients||[]).filter(c=>String(c.status||'').toLowerCase()==='active')){
        ...
        for(const weekStart of weeks){
          ...
          if(weekHasOccurrence(client.id,weekStart)||basketHasOccurrence(client.id,weekStart)||movedOccurrenceExistsV6058(client.id,weekStart))continue;
          if(cloudBlocksOccurrenceV6058(cloudIndex,client.id,weekStart)){skippedCloud++;continue;}
      ...
      if(!candidates.length){rollingLastRun=Date.now();window.__tuinbooksRollingV6058Last={reason,created:0,skippedNew,skippedNoPattern,skippedCloud,at:new Date().toISOString()};return window.__tuinbooksRollingV6058Last;}

      let pending=[...candidates],savedResult=null;
      try{savedResult=await saveRollingCandidatesV6058(pending);}
      catch(error){
        if(!recurrenceCollisionV6058(error))throw error;
        ...
        cloudRows=await cloudScheduleHorizonV6058(firstWeek,horizonEnd);cloudIndex=cloudOccurrenceIndexV6058(cloudRows);
        pending=pending.filter(job=>!cloudBlocksOccurrenceV6058(cloudIndex,job.clientId,mondayOf(job.date)));
        if(pending.length)savedResult=await saveRollingCandidatesV6058(pending);else savedResult={saved:[],revision:Number((window.backendV28||{}).operationalRevision||0)};
      }
      adoptSavedCandidatesV6058(savedResult.saved,savedResult.revision);
      try{window.renderSchedule?.();}catch(_){ }
      setTimeout(decorateScheduleCards,0);
      rollingLastRun=Date.now();
      window.__tuinbooksRollingV6058Last={reason,created:savedResult.saved.length,skippedNew,skippedNoPattern,skippedCloud,reconciled:candidates.length-savedResult.saved.length,at:new Date().toISOString()};
      console.info('[TuinBooks v60.5.8] cloud-aware rolling schedule',window.__tuinbooksRollingV6058Last);
      return window.__tuinbooksRollingV6058Last;
    }catch(error){
      console.error('[TuinBooks v60.5.8] rolling schedule stopped safely',error);
  ...
  window.maintainRollingScheduleV6058=maintainRollingScheduleV6058;
  function scheduleRollingMaintenance(reason,delay=350){
    clearTimeout(window.__tuinbooksRollingV6058Timer);
    window.__tuinbooksRollingV6058Timer=setTimeout(()=>maintainRollingScheduleV6058(reason),delay);
  }
  function waitForRollingWorkspaceV6058(reason='startup'){
    clearInterval(window.__tuinbooksRollingReadyPollV6058);
    const started=Date.now();
    const attempt=()=>{
      if(rollingWorkspaceReadyV6058()){
        clearInterval(window.__tuinbooksRollingReadyPollV6058);window.__tuinbooksRollingReadyPollV6058=null;
        scheduleRollingMaintenance(reason,180);return true;
      }
      if(Date.now()-started>90000){clearInterval(window.__tuinbooksRollingReadyPollV6058);window.__tuinbooksRollingReadyPollV6058=null;}
      return false;
    };
    if(attempt())return;
    window.__tuinbooksRollingReadyPollV6058=setInterval(attempt,500);
  }
  ...
  function install() {
    ...
    // Rolling maintenance is desktop-only and MUST wait for the cloud workspace to finish hydrating.
    // This prevents locally-created visits from briefly appearing and then being replaced by the real Supabase load.
    if (document.body?.dataset?.app !== 'mobile') {
      waitForRollingWorkspaceV6058('workspace-hydrated');
      document.addEventListener('click',event=>{
        if (event.target?.closest?.('.nav-tab[data-view="schedule"], [data-view="schedule"]')) waitForRollingWorkspaceV6058('schedule-open');
      },true);
      document.addEventListener('drop',event=>{
        if (event.target?.closest?.('#weeklyScheduleBoard,.schedule-day-lane')) waitForRollingWorkspaceV6058('schedule-drop');
      },true);
      setInterval(()=>{
        if(document.visibilityState==='visible'&&Date.now()-rollingLastRun>60000)waitForRollingWorkspaceV6058('visible-safety');
      },75000);
    }
  ...
  window.__tuinbooksSchedulePolishV6058Test={explicitCadence,inferCadence,clientPattern,targetForWeek,rollingWorkspaceReadyV6058,maintainRollingScheduleV6058,cloudOccurrenceIndexV6058,cloudBlocksOccurrenceV6058};
