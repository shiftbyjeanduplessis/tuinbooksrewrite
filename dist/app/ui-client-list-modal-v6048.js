...
(()=>{
  'use strict';
  const BUILD='60.4.8.1-new-r-basket-persistence';
  ...
  function wrapRenderClients(){
    ...
    const wrapped=function(...args){const result=base.apply(this,args);queueDecorate();setTimeout(()=>{void repairMissingNewRBasketV60481();},0);return result;};
  ...
  function bindArchiveClose(){
    let button=$('archiveClientBtn');if(!button)return;
    if(button.dataset.lifecycleV6048==='1')return;
    if(typeof window.archiveClientV6048==='function'){
      const clean=button.cloneNode(true);button.replaceWith(clean);button=clean;button.dataset.lifecycleV6048='1';
      button.addEventListener('click',async event=>{
        event.preventDefault();event.stopImmediatePropagation();
        const id=$('clientId')?.value||'';if(!id){if(typeof toast==='function')toast('Choose a client first.','error');return;}
        const archived=await window.archiveClientV6048(id,{confirmUser:true});
  ...
  async function activateDirectV6045(clientForm,submit){
    ...
    try{
      ...
      try{
        if(typeof backendV28!=='undefined'&&backendV28?.mode==='supabase'){
          if(typeof syncCoreDeltaV59395==='function')await syncCoreDeltaV59395(true);
          // NEW R lives in the operational document (Schedule Basket), not the core client row.
          // Persist both halves together so a reload cannot keep the badge but lose the basket item.
          if(typeof syncOperationalDeltaV59394==='function')await syncOperationalDeltaV59394(true);
          else if(typeof syncOperationalSnapshotV41==='function')await syncOperationalSnapshotV41(true);
        }
      }catch(error){console.warn('[v60.4.8.1] active client NEW R cloud sync',error);}
      ...
      submit.disabled=false;submit.textContent=primaryLabelV6048();
    }
  }
  ...
  let newRRepairBusyV60481=false;
  async function repairMissingNewRBasketV60481(){
    if(newRRepairBusyV60481)return false;
    newRRepairBusyV60481=true;
    try{
      const app=appStateV6045(),today=todayISO();
      if(!app||!Array.isArray(app.clients))return false;
      ...
      let coreChanged=false,operationalChanged=false,restored=0,removed=0;
      const recurring=/^(Weekly|Fortnightly|Monthly)$/;
      for(const client of app.clients){
        if(client?.awaitingInitialRecurringPlacementV6036!==true)continue;
        if(String(client.status||'').toLowerCase()!=='active')continue;
        if(!recurring.test(String(client.frequency||'')))continue;

        const clientJobs=app.schedules.filter(job=>String(job?.clientId||'')===String(client.id)&&String(job?.status||'scheduled').toLowerCase()!=='cancelled');
        const realFirstPlacement=clientJobs.some(job=>job?.initialRecurringPlacementV6036===true);
        if(realFirstPlacement){
          // A genuine drag from NEW R already happened; repair only the stale flag.
          client.awaitingInitialRecurringPlacementV6036=false;
          coreChanged=true;
          continue;
        }

        // While NEW R is waiting, there must be no automatically generated future
        // recurring visit. Remove only rolling/auto-generated rows; never touch
        // completed history or a genuine manually placed first visit.
        const staleIds=new Set(clientJobs.filter(job=>{
          const d=String(job?.date||'').slice(0,10);
          if(!d||d<today)return false;
          const status=String(job?.status||'scheduled').toLowerCase();
          if(['completed','missed','cancelled','canceled'].includes(status))return false;
          const recurringJob=job?.workMarker==='R'||job?.workKind==='recurring'||job?.rollingGeneratedV58929===true||Boolean(job?.recurrenceKey);
          const generated=job?.autoGenerated===true||job?.rollingGeneratedV58929===true||Boolean(job?.recurrenceKey)||String(job?.sourceOccurrenceKey||'').startsWith('rolling:');
          return recurringJob&&generated&&job?.initialRecurringPlacementV6036!==true;
        }).map(job=>String(job.id)));
        if(staleIds.size){
          app.schedules=app.schedules.filter(job=>!staleIds.has(String(job?.id||'')));
          removed+=staleIds.size;operationalChanged=true;
        }

        const hasBasket=app.scheduleBasket.some(item=>item?.newRecurringV6036===true&&String(item?.clientId||item?.jobPayload?.clientId||'')===String(client.id));
        if(!hasBasket){
          const item=ensureNewRBasketV6045(client);
          if(item){restored++;operationalChanged=true;coreChanged=true;}
        }else{
          // Keep NEW R calendar-owned: no date/team anchor until the office drags it.
          if(client.serviceStartDate||client.recurrenceAnchorDate||client.preferredDay||client.teamId||client.preferredTeamId){
            client.serviceStartDate='';client.recurrenceAnchorDate='';client.preferredDay='';client.teamId='';client.preferredTeamId='';coreChanged=true;
          }
        }
      }
      if(!coreChanged&&!operationalChanged)return false;
      try{if(typeof save==='function')save();}catch(_){ }
      if(typeof backendV28!=='undefined'&&backendV28?.mode==='supabase'){
        if(coreChanged&&typeof syncCoreDeltaV59395==='function')await syncCoreDeltaV59395(true);
        if(operationalChanged){
          if(typeof syncOperationalDeltaV59394==='function')await syncOperationalDeltaV59394(true);
          else if(typeof syncOperationalSnapshotV41==='function')await syncOperationalSnapshotV41(true);
        }
      }
      try{if(typeof renderScheduleQueue==='function')renderScheduleQueue();}catch(_){ }
      if(restored||removed)console.info('[TuinBooks v60.4.8.1] NEW R basket repaired',{restored,removed});
      return true;
    }catch(error){
      console.warn('[v60.4.8.1] NEW R basket repair deferred',error);
      return false;
    }finally{newRRepairBusyV60481=false;}
  }
  ...
  function bindReliableSubmitV6045(){
    ...
    const run=()=>{
      ...
      if(isNewClientV6048()&&activatingClientV6045()){
        openActivationDialogV6045(()=>{void activateDirectV6045(clientForm,submit);});
      }else if(!isNewClientV6048()&&typeof window.saveExistingClientV6048==='function'){
        void window.saveExistingClientV6048(clientForm,submit);
  ...
  function install(){
    ...
    // Run after both initial UI paint and tenant hydration. Re-running is idempotent.
    setTimeout(()=>{void repairMissingNewRBasketV60481();},1200);
    setTimeout(()=>{void repairMissingNewRBasketV60481();},4200);
    // Older release markers and wrappers settle after startup. Re-assert only this Clients shell.
    setTimeout(()=>{wrapEditClient();wrapRenderClients();addNewClientButton();bindClose();bindArchiveClose();observeList();bindReliableSubmitV6045();refreshPrimaryLabelV6048();removeLegacyPagination();queueDecorate();markBuild();},900);
    setTimeout(()=>{bindArchiveClose();refreshPrimaryLabelV6048();removeLegacyPagination();queueDecorate();markBuild();},3500);
    window.__tuinbooksClientListModalV6048={build:BUILD,listOnlyPage:true,recordSeparateModal:true,paginationRemoved:true,statusDetail:true,directActivatePath:true,newRBasketCloudPersistence:true,repairMissingNewRBasketV60481,frozenScheduleUntouched:true};
  }
