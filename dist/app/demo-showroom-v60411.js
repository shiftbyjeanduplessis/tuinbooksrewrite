...
(()=>{
  'use strict';
  const BUILD='60.4.11-demo-showroom-midday-guide';
  ...
  const TOUR_STORAGE_PREFIX='tuinbooks-demo-tour-v60411:';
  let tourIndex=-1;
  ...
  function buildShowroomState(){
    ...
    const liveDayClients=clients.slice(0,8);
    liveDayClients.forEach((client,index)=>{
      const teamId=index<4?proteaId:aloeId;
      const sort=(index%4)+1;
      const status=index<=2||index===4?'completed':index===5?'attention':'scheduled';
      ...
      if(status==='completed'||status==='attention'){
        ...
        visits.push({id:`demo-live-visit-${index+1}`,clientId:client.id,date:today,teamId,scheduled:true,scheduledJobId:jobId,workDone:status==='attention'?['Cut grass','Edge lawns and paths']:client.serviceIds.includes('lawn-mowing')?['Cut grass','Edge lawns and paths','Weed and tidy beds','General garden tidy']:['Weed and tidy beds','Prune agreed plants','General garden tidy'],extraDescription:status==='attention'?'Team could not finish the rear hedge because access was temporarily blocked.':'',outcome:status==='attention'?'Partially completed':'Completed',outcomeNote:status==='attention'?'Rear hedge section still needs a decision from the office.':'',processed:status==='completed',resolutionStatusV56:status==='attention'?'needs-resolution':'processed',photoPaths:[],photos:[photo],billingProfileIdV59396:profileId,completedAt:`${today}T${index<4?'10':'11'}:${String(10+index*5).padStart(2,'0')}:00.000Z`,createdAt:`${today}T${index<4?'10':'11'}:${String(10+index*5).padStart(2,'0')}:00.000Z`});
      }
    });
  ...
  function applyDemoPhotosToState(){
    ...
    try{if(activeViewName()==='records')window.renderRecords?.();}catch(_){ }
  }
  ...
  const VIEW_GUIDES={
    schedule:{title:'Start with the live schedule',copy:'You are looking at a working week for two field teams. Today is deliberately shown mid-shift so you can see completed jobs, remaining work and an exception. Open a job or drag around the demo freely.'},
    records:{title:'See what is happening today',copy:'Work is the office control centre during the day. The demo shows today’s completion percentage, team progress, field photos and one visit needing attention.'},
    clients:{title:'Open a real-looking client',copy:'These are sample residential, estate and commercial clients. Open one to see services, recurrence, instructions and billing information.'},
    quotes:{title:'Follow a quote through the pipeline',copy:'The demo includes Draft, Sent, Accepted and Completed examples. Open any quote and then return here to keep exploring.'},
    invoices:{title:'See how work reaches billing',copy:'Billing contains routine drafts, quoted work ready to invoice, and examples of sent and paid invoices. Nothing in this demo is sent to a real customer.'}
  };
  function activeViewName(){const active=document.querySelector('.app-view.active');return active?.id?.replace(/^view-/,'')||'';}
  function installViewGuide(force=false){
    if(!isDemo())return;
    const viewName=activeViewName(),info=VIEW_GUIDES[viewName],view=document.querySelector('.app-view.active');
    document.querySelectorAll('.tb-demo-view-guide-v60411').forEach(node=>node.remove());
    if(!view||!info)return;
    const sessionKey=`tuinbooks-demo-guide-dismissed-v60411:${backend()?.businessId||'demo'}`;
    if(!force){try{if(sessionStorage.getItem(sessionKey)==='1')return;}catch(_){ }}
    const guide=document.createElement('aside');guide.className='tb-demo-view-guide-v60411';
    guide.innerHTML=`<button type="button" class="tb-demo-close-icon-v60410" aria-label="Close demo guide">×</button><span>Demo guide</span><strong>${esc(info.title)}</strong><p>${esc(info.copy)}</p><button type="button" class="button secondary compact" data-demo-tour-from-guide>Show guided tour</button>`;
    const heading=view.querySelector('.page-heading');
    if(heading?.nextSibling)view.insertBefore(guide,heading.nextSibling);else view.prepend(guide);
    guide.querySelector('.tb-demo-close-icon-v60410')?.addEventListener('click',()=>{try{sessionStorage.setItem(sessionKey,'1');}catch(_){ }guide.remove();});
    guide.querySelector('[data-demo-tour-from-guide]')?.addEventListener('click',()=>showWelcome(true));
  }
  function todayProgress(){
    const s=currentState()||{},today=isoLocal(),jobs=(s.schedules||[]).filter(job=>job.date===today&&!['cancelled','canceled'].includes(String(job.status||'').toLowerCase()));
    const completed=jobs.filter(job=>String(job.status||'').toLowerCase()==='completed'||(s.visits||[]).some(v=>v.scheduledJobId===job.id&&String(v.outcome||'').toLowerCase()==='completed')).length;
    const unresolved=jobs.filter(job=>['attention','missed','unresolved'].includes(String(job.status||'').toLowerCase())||(s.visits||[]).some(v=>v.scheduledJobId===job.id&&!['completed','done'].includes(String(v.outcome||'').toLowerCase()))).length;
    const remaining=Math.max(0,jobs.length-completed-unresolved);
    const pct=jobs.length?Math.round(completed/jobs.length*100):0;
    const teams=(s.teams||[]).filter(t=>t.active!==false).map(team=>{const tj=jobs.filter(j=>j.teamId===team.id),tc=tj.filter(j=>String(j.status||'').toLowerCase()==='completed'||(s.visits||[]).some(v=>v.scheduledJobId===j.id&&String(v.outcome||'').toLowerCase()==='completed')).length;return {name:team.name,total:tj.length,completed:tc,pct:tj.length?Math.round(tc/tj.length*100):0};}).filter(t=>t.total);
    return {total:jobs.length,completed,unresolved,remaining,pct,teams};
  }
  function installWorkShowcase(){
    if(!isDemo())return;
    const view=$('view-records');if(!view)return;
    $('tbDemoWorkSummaryV60411')?.remove();
    const p=todayProgress();
    const card=document.createElement('section');card.id='tbDemoWorkSummaryV60411';card.className='tb-demo-work-summary-v60411';
    const visitPhotos=(currentState()?.visits||[]).filter(v=>v.date===isoLocal()&&Array.isArray(v.photos)&&v.photos[0]).slice(0,4);
    card.innerHTML=`<div class="tb-demo-work-summary-head-v60411"><div><span>Today’s live progress</span><strong>${p.pct}% complete</strong><p>${p.completed} completed · ${p.remaining} remaining${p.unresolved?` · ${p.unresolved} needs attention`:''}</p></div><div class="tb-demo-progress-ring-v60411" style="--p:${p.pct}"><b>${p.pct}%</b></div></div><div class="tb-demo-team-progress-v60411">${p.teams.map(t=>`<div><span>${esc(t.name)}</span><strong>${t.completed}/${t.total}</strong><i><em style="width:${t.pct}%"></em></i><small>${t.pct}% complete</small></div>`).join('')}</div>${visitPhotos.length?`<div class="tb-demo-photo-strip-v60411"><div><span>Field evidence received today</span><small>Sample photos from completed visits</small></div><div class="tb-demo-photo-grid-v60411">${visitPhotos.map((v,i)=>`<img src="${esc(v.photos[0])}" alt="Sample completed garden work ${i+1}">`).join('')}</div></div>`:''}`;
    const heading=view.querySelector('.page-heading');
    const guide=view.querySelector('.tb-demo-view-guide-v60411');
    const after=guide||heading;
    if(after?.nextSibling)view.insertBefore(card,after.nextSibling);else view.prepend(card);
  }
  function refreshDemoGuidance(force=false){
    installViewGuide(force);
    if(activeViewName()==='records')installWorkShowcase();
  }

  function installHeaderControls(){
    if(!isDemo()||document.querySelector('.tb-demo-tour-btn-v6049'))return;
    const host=document.querySelector('.header-actions')||document.querySelector('.admin-header')||document.querySelector('header');if(!host)return;
  ...
  function installWhenReady(){
    let tries=0;const timer=setInterval(()=>{
      ...
      if(isDemo()&&backend()?.businessId){
        clearInterval(timer);installResetButton();installHeaderControls();applyDemoPhotosToState();refreshDemoGuidance();setTimeout(()=>showWelcome(false),500);
      }else if(tries>80)clearInterval(timer);
    },250);
  }

  window.installTuinBooksDemoShowroomV6049=restoreShowroom;
  document.addEventListener('click',event=>{if(event.target.closest('.nav-tab,[data-view]'))setTimeout(()=>refreshDemoGuidance(),120);});
  const demoViewObserver=new MutationObserver(()=>{if(isDemo())setTimeout(()=>refreshDemoGuidance(),80);});
  if(document.body)demoViewObserver.observe(document.body,{subtree:true,attributes:true,attributeFilter:['class']});
  ...
  window.addEventListener('tuinbooks:runtime-ready',()=>setTimeout(()=>{installResetButton();installHeaderControls();applyDemoPhotosToState();refreshDemoGuidance();showWelcome(false);},120));
})();
