/* TuinBooks v60.7.26 — Schedule UI stability guard
   DOM-only, loaded last. Does not touch schedule data or persistence. */
(()=>{
  'use strict';
  const BUILD='60.7.26-production-stability';
  ...
  window.closeScheduleQueue=function closeScheduleQueueUiV60726(){
    try{closeBasketBeforeV60726?.();}catch(error){console.warn('[TuinBooks v60.7.26] basket close fallback',error);}
    closeBasketToTag();
  };
  ...
  function ensureFloatingClose(){
    let button=document.getElementById('scheduleDetailFloatingCloseV60726');
    if(button)return button;
    button=document.createElement('button');
    button.id='scheduleDetailFloatingCloseV60726';
    button.type='button';
    button.className='tb-schedule-detail-floating-close-v60726';
  ...
  function syncDetailExit(){
    ...
    document.documentElement.style.setProperty('--tb-detail-top-v60726',`${top}px`);
    button.style.top=`${top}px`;
  }

  function installStyles(){
    if(document.getElementById('scheduleUiStabilityStylesV60726'))return;
    const style=document.createElement('style');
    style.id='scheduleUiStabilityStylesV60726';
    style.textContent=`
      #scheduleDetailPanel:not(.hidden){
        top:var(--tb-detail-top-v60726,10px)!important;
        bottom:8px!important;
        height:auto!important;
        max-height:calc(100dvh - var(--tb-detail-top-v60726,10px) - 8px)!important;
      }
      .tb-schedule-detail-floating-close-v60726{
      ...
      .tb-schedule-detail-floating-close-v60726[hidden]{display:none!important}
      #tuinbooksBuildMarkerV60726{
  ...
  function installBuildMarker(){
    let marker=document.getElementById('tuinbooksBuildMarkerV60726');
    if(!marker){marker=document.createElement('div');marker.id='tuinbooksBuildMarkerV60726';document.body.appendChild(marker);}
    marker.textContent='BUILD v60.7.26';
    marker.title='TuinBooks production build 60.7.26';
  }
  ...
  window.__tuinbooksProductionStabilityV60726={build:BUILD,closeScheduleDetail,removeOnboardingShortcut};
