...
(()=>{
  ...
  const BUILD='60.0.11-schedule-action-permission-fix';
  ...
  function installStatus(){
    ...
    const update=()=>{
      ...
      el.innerHTML=`<strong>v60.0.11 ACTION SAVE FIX</strong> · ${location.pathname.endsWith('consolidation-v6011.html')||location.pathname.endsWith('mobile-v6011.html')?'unique page YES':'WRONG PAGE'} · ${(st.schedules||[]).length} jobs · ${(st.teams||[]).length} teams · ${actions.length} saved actions · ${mobile?`mobile first-item ${mobileFirst?'YES':'waiting'}`:`${controls} team-day action sets`} · production untouched`;
    };
  ...
  function boot(){
    ...
    installStatus();
    console.info('[TuinBooks v60.0.11 canary]',{build:BUILD,calendarActions:true,mobileInstructionFirst:true,recurringMovePrompt:true,productionUntouched:true});
  }
