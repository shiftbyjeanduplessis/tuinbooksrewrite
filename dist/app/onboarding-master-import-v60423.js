/* ========================================================================== 
   ...
   ========================================================================== */
(()=>{
  const BUILD='60.4.24-onboarding-import-self-closing-cell-fix';
  ...
  const DAYS_V60423=['Monday','Tuesday','Wednesday','Thursday','Friday','Saturday','Sunday'];
  let state=null;
  const runtimeV60423=()=>window.__tuinbooksOnboardingRuntimeV60423||null;
  function syncStateV60423(){
    const runtime=runtimeV60423();
    const current=runtime?.getState?.();
    if(!current)throw new Error('TuinBooks is still loading this business. Wait for the account to finish loading, then try the import again.');
    state=current;
    return state;
  }
  function localDateV60423(){const d=new Date(),y=d.getFullYear(),m=String(d.getMonth()+1).padStart(2,'0'),day=String(d.getDate()).padStart(2,'0');return `${y}-${m}-${day}`;}
  function dateAddV60423(iso,days){const d=new Date(`${iso}T12:00:00`);d.setDate(d.getDate()+Number(days||0));return `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}-${String(d.getDate()).padStart(2,'0')}`;}
  function startOfWeekV60423(iso){const d=new Date(`${iso}T12:00:00`),day=d.getDay();return dateAddV60423(iso,-((day+6)%7));}
  function weekDatesV60423(weekStart){return DAYS_V60423.map((_,i)=>dateAddV60423(weekStart,i));}
  function dayNameV60423(iso){const jsDay=new Date(`${iso}T12:00:00`).getDay();return DAYS_V60423[jsDay-1]||'Sunday';}
  ...
  function dayV60420(value,date=''){
    const text=cleanV60420(value),match=DAYS_V60423.find(day=>keyV60420(day)===keyV60420(text));
    if(match)return match;
    if(date){const derived=dayNameV60423(date);return DAYS_V60423.includes(derived)?derived:'';}
    return '';
  }
  ...
  function currentProfileMatchV60420(value){
    ...
    const profiles=runtimeV60423()?.billingProfiles?.()||(Array.isArray(state?.billingProfilesV59396)?state.billingProfilesV59396:[]);
  ...
  /* ---------- compatibility: earlier TuinBooks trial workbooks ---------- */
  function legacyHeaderObjectsV60421(rows){return headerObjectsV60420(rows||[]);}
  function legacySheetV60421(parsed,names,kind=''){
    const entries=Object.entries(parsed?.sheets||{}),wanted=new Set(names.map(name=>keyV60420(name)));
    for(const [name,rows] of entries)if(wanted.has(keyV60420(name)))return {name,rows};
    for(const [name,rows] of entries){
      const objects=legacyHeaderObjectsV60421(rows),sample=objects[0]||{},keys=new Set(Object.keys(sample).map(keyV60420));
      if(kind==='clients'&&keys.has('name')&&keys.has('address')&&(keys.has('service scope')||keys.has('team name')))return {name,rows};
      if(kind==='schedule'&&(keys.has('preferred day')||keys.has('date')||keys.has('service date'))&&(keys.has('name')||keys.has('client')||keys.has('client name'))&&(keys.has('schedule source')||keys.has('team id')||keys.has('preferred team id')))return {name,rows};
      if(kind==='teams'&&keys.has('team name')&&(keys.has('crew 1')||keys.has('mobile pin')||keys.has('team id')))return {name,rows};
    }
    return null;
  }
  function legacyValueV60421(row,...names){return rowGetV60420(row,...names);}
  function legacyActiveV60421(row){const value=legacyValueV60421(row,'active','Active');const parsed=yesNoV60420(value);return parsed===null?true:parsed;}
  function legacyDayV60421(value){const text=keyV60420(value),days=['Monday','Tuesday','Wednesday','Thursday','Friday','Saturday','Sunday'];return days.find(day=>keyV60420(day)===text)||'';}
  function legacyCurrentClientsV60421(){return Array.isArray(state?.clients)?state.clients:[];}
  function legacyCurrentTeamsV60421(){return Array.isArray(state?.teams)?state.teams:[];}
  function legacyClientMatchExistingV60421(row){
    const rows=legacyCurrentClientsV60421(),idx=cleanV60420(legacyValueV60421(row,'index','client id','Client ID','client number')),name=keyV60420(legacyValueV60421(row,'name','client','client name','Client / Site Name')),address=keyV60420(legacyValueV60421(row,'address','Street Address'));
    if(idx){const byRef=rows.find(client=>[client.onboardingLegacyIndexV60421,client.accountReference,client.customerReference,client.onboardingClientIdV60420].some(value=>keyV60420(value)===keyV60420(idx)));if(byRef)return byRef;}
    if(address){const byAddress=rows.find(client=>keyV60420(client.address)===address);if(byAddress)return byAddress;}
    if(name){const candidates=rows.filter(client=>keyV60420(client.name)===name);if(candidates.length===1)return candidates[0];}
    return null;
  }
  function legacyTeamMatchExistingV60421(value){const key=keyV60420(value);if(!key)return null;return legacyCurrentTeamsV60421().find(team=>keyV60420(team.id)===key||keyV60420(team.name)===key||keyV60420(team.leaderName)===key)||null;}
  function legacyModelV60421(parsed){
    const clientsSheet=legacySheetV60421(parsed,['Clients Import','Clients','CLIENTS'],'clients');
    const scheduleSheet=legacySheetV60421(parsed,['Schedule Import','Schedule','SCHEDULE'],'schedule');
    const teamsSheet=legacySheetV60421(parsed,['Team Setup','Teams','TEAMS'],'teams');
    const isMaster=Object.keys(parsed?.sheets||{}).some(name=>keyV60420(name)==='start');
    if(isMaster||(!clientsSheet&&!scheduleSheet&&!teamsSheet))return null;
    const clients=clientsSheet?legacyHeaderObjectsV60421(clientsSheet.rows).filter(row=>cleanV60420(legacyValueV60421(row,'name','Client / Site Name','client'))||cleanV60420(legacyValueV60421(row,'address','Street Address'))):[];
    const schedules=scheduleSheet?legacyHeaderObjectsV60421(scheduleSheet.rows).filter(row=>cleanV60420(legacyValueV60421(row,'name','client','client name','Client / Site Name'))||cleanV60420(legacyValueV60421(row,'address'))):[];
    const teams=teamsSheet?legacyHeaderObjectsV60421(teamsSheet.rows).filter(row=>cleanV60420(legacyValueV60421(row,'team_name','team name','Team Name'))):[];
    const issues=[],warnings=[];
    if(!clients.length&&!schedules.length&&!teams.length)issues.push('No usable client, schedule or team rows were found in this workbook.');
    if(schedules.length){
      const workbookNames=new Map();clients.forEach(row=>{const idx=cleanV60420(legacyValueV60421(row,'index','client id'));const name=keyV60420(legacyValueV60421(row,'name','client'));const address=keyV60420(legacyValueV60421(row,'address'));if(idx)workbookNames.set(`id:${keyV60420(idx)}`,row);if(name)workbookNames.set(`name:${name}`,row);if(address)workbookNames.set(`address:${address}`,row);});
      schedules.forEach(row=>{
        if(!legacyActiveV60421(row))return;
        const idx=cleanV60420(legacyValueV60421(row,'index','client id')),name=keyV60420(legacyValueV60421(row,'name','client','client name')),address=keyV60420(legacyValueV60421(row,'address'));
        const inBook=(idx&&workbookNames.has(`id:${keyV60420(idx)}`))||(address&&workbookNames.has(`address:${address}`))||(name&&workbookNames.has(`name:${name}`));
        if(!inBook&&!legacyClientMatchExistingV60421(row))issues.push(`Schedule row ${row._row}: ${cleanV60420(legacyValueV60421(row,'name','client','client name'))||`row ${row._row}`} does not match a client in this workbook or the open TuinBooks account.`);
        const day=cleanV60420(legacyValueV60421(row,'preferred_day','preferred day','Preferred Day')),date=excelDateV60420(legacyValueV60421(row,'date','service date','recurrence_anchor','recurrence anchor'));
        if(!date&&!legacyDayV60421(day))issues.push(`Schedule row ${row._row}: Preferred Day must be Monday to Sunday, or provide a service date.`);
      });
      if(!teams.length&&!legacyCurrentTeamsV60421().length)issues.push('Schedule import needs a team. Add Team Setup to the workbook or create a team in TuinBooks first.');
    }
    if(!clients.length&&schedules.length)warnings.push('Schedule-only workbook: existing TuinBooks clients will be matched by client number, address or name.');
    if(!teams.length&&legacyCurrentTeamsV60421().length)warnings.push('No Team Setup sheet found; schedule rows will use their named team when matched, otherwise the current TuinBooks team.');
    warnings.push('Earlier TuinBooks trial workbook detected. Missing phone, email, rate, frequency and recurrence fields are allowed and will not be guessed.');
    warnings.push('Preferred-day trial rows are placed once in the currently selected schedule week. They are not turned into an invented recurring pattern.');
    const currentName=cleanV60420(state?.business?.name||'')||cleanV60420(parsed?.fileName).replace(/[_-]+/g,' ').replace(/\b(schedule|client|clients|trial|import|tuinbooks|xlsx)\b/gi,' ').replace(/\s+/g,' ').trim()||'Current business';
    return {legacy:true,parsed,start:{},trial:'Planning only',financial:false,businessName:currentName,clients,teams,schedules,billing:[],items:[],issues:[...new Set(issues)],warnings:[...new Set(warnings)],counts:{clients:clients.length,teams:teams.length,schedules:schedules.filter(legacyActiveV60421).length,billing:0,items:0},legacySheets:{clients:clientsSheet?.name||'',schedule:scheduleSheet?.name||'',teams:teamsSheet?.name||''}};
  }
  function modelV60420(parsed){
    const legacy=legacyModelV60421(parsed);if(legacy)return legacy;
    ...
    const currentName=cleanV60420(state?.business?.name||'');if(businessName&&currentName&&!businessNameCompatibleV60420(businessName,currentName))issues.push(`This workbook is for “${businessName}”, but the open TuinBooks account is “${currentName}”. Open the correct account or correct START → Business name.`);
    ...
    schedules.forEach(row=>{
      ...
      if(!preferred)issues.push(`${prefix}: Preferred Day must be Monday to Sunday.`);
  ...
  if(typeof window==='undefined'||typeof document==='undefined'){
    globalThis.__tuinbooksOnboardingMasterImportV60420={...publicParser,modelV60420,legacyModelV60421};
    return;
  }
  ...
  function workspaceWriteReadyV60420(){
    try{
      syncStateV60423();
      const backend=runtimeV60423()?.getBackend?.();
      if(!backend)return true;
      const params=new URLSearchParams(location.search),management=params.get('support')==='1'&&!!params.get('business');
      if(management){
        if(backend.managementCoreReadyV5950===false)return false;
        const operationalRequired=backend.managementOperationalLoadRequiredV59371===true;
        const operationalReady=backend.managementOperationalReadyV5950===true||backend.managementOperationalReadyV59371===true;
        if(operationalRequired&&!operationalReady)return false;
      }
      if(backend.coreConflict||backend.operationalConflict)return false;
      return true;
    }catch(_){return false;}
  }
  function workspaceWriteIssueV60420(){
    try{const backend=runtimeV60423()?.getBackend?.();if(backend&&(backend.coreConflict||backend.operationalConflict))return 'This workspace has a cloud-save conflict. Reload the account before importing.';}catch(_){ }
    if(!runtimeV60423()?.getState?.())return 'TuinBooks is still loading this business. Wait for the account to finish loading, then try again.';
  ...
  function applyTeamsV60420(model){
    ...
    model.teams.forEach((row,index)=>{
      ...
      if(!team){let id=safeImportIdV60420(workbookId,'team');if(state.teams.some(item=>item.id===id))id=runtimeV60423()?.uid?.('team')||`team-${Date.now()}-${index}`;team={id,capacityHours:8,bufferHours:0,dailySiteCapacity:0,createdAt:now};state.teams.push(team);}
  ...
  function dueForWeekV60420(def,weekStart,dates){
    const appDue=runtimeV60423()?.recurringFrequencyDue?.(def.frequency,def.anchor,def.preferred,weekStart,dates);if(typeof appDue==='boolean')return appDue;
    const target=dates[Math.max(0,DAYS_V60423.indexOf(def.preferred))]||dates[0];if(target<def.anchor)return false;if(def.frequency==='Weekly')return true;if(def.frequency==='Fortnightly'){const weeks=Math.floor((new Date(`${weekStart}T12:00:00`)-new Date(`${startOfWeekV60423(def.anchor)}T12:00:00`))/(7*86400000));return weeks>=0&&weeks%2===0;}return target.slice(0,7)===def.anchor.slice(0,7);
  }
  function seedScheduleHorizonV60420(definitions){
    state.schedules=Array.isArray(state.schedules)?state.schedules:[];const today=localDateV60423(),first=startOfWeekV60423(today),horizonEnd=dateAddV60423(first,IMPORT_HORIZON_WEEKS*7-1),stats={created:0,updated:0,preserved:0};
    definitions.forEach(def=>{
      for(let weekIndex=0;weekIndex<IMPORT_HORIZON_WEEKS;weekIndex++){
        const weekStart=dateAddV60423(first,weekIndex*7),dates=weekDatesV60423(weekStart);if(!dueForWeekV60420(def,weekStart,dates))continue;
        const date=dates[Math.max(0,DAYS_V60423.indexOf(def.preferred))]||dates[0];if(date<today||date>horizonEnd)continue;
        ...
        let job=(state.schedules||[]).find(item=>item.clientId===def.clientId&&startOfWeekV60423(item.date||today)===weekStart&&((item.recurrenceKey&&item.recurrenceKey===recurrenceKey)||(keyV60420(runtimeV60423()?.workMarkerForJob?.(item))==='r'||item.workKind==='recurring')));
        ...
        else{job={id:runtimeV60423()?.uid?.('sch')||`sch-${Date.now()}-${stats.created}`,createdAt:new Date().toISOString(),...values};state.schedules.push(job);stats.created++;}
      }
    });
    try{for(let weekIndex=0;weekIndex<IMPORT_HORIZON_WEEKS;weekIndex++)runtimeV60423()?.normaliseRouteOrder?.(weekDatesV60423(dateAddV60423(first,weekIndex*7)));}catch(_){ }
    return stats;
  }
  function applyBillingV60420(model,clientMap){
    ...
    model.billing.forEach(row=>{
      ...
      runtimeV60423()?.applyBillingClassification?.(client);
    });
  }
  ...
  function legacyTeamKeyV60421(row){return cleanV60420(legacyValueV60421(row,'team_id','team id','Team ID','team_name','team name','Team Name'));}
  function applyLegacyTeamsV60421(model){
    state.teams=Array.isArray(state.teams)?state.teams:[];const map=new Map(),now=new Date().toISOString();
    model.teams.forEach((row,index)=>{
      const rawId=cleanV60420(legacyValueV60421(row,'team_id','team id','Team ID')),name=cleanV60420(legacyValueV60421(row,'team_name','team name','Team Name'))||`Team ${index+1}`;
      let team=(rawId&&legacyTeamMatchExistingV60421(rawId))||legacyTeamMatchExistingV60421(name);
      if(!team){let id=safeImportIdV60420(rawId||name,'team');if(state.teams.some(item=>item.id===id))id=runtimeV60423()?.uid?.('team')||`team-${Date.now()}-${index}`;team={id,name,active:true,createdAt:now};state.teams.push(team);}
      const crew=[1,2,3,4].map(n=>cleanV60420(legacyValueV60421(row,`crew_${n}`,`crew ${n}`))).filter(Boolean);
      Object.assign(team,{name,active:legacyActiveV60421(row),leaderName:team.leaderName||crew[0]||'',crewMembersV60421:crew,mobilePinV60421:cleanV60420(legacyValueV60421(row,'mobile_pin','mobile pin')),notes:cleanV60420(legacyValueV60421(row,'notes')),onboardingLegacyTeamV60421:true,updatedAt:now});
      [rawId,name].filter(Boolean).forEach(value=>map.set(keyV60420(value),team.id));
    });
    (state.teams||[]).forEach(team=>{map.set(keyV60420(team.id),team.id);map.set(keyV60420(team.name),team.id);});
    return map;
  }
  function legacyClientMatchStateV60421(row,claimed=new Set()){
    const idx=cleanV60420(legacyValueV60421(row,'index','client id','Client ID')),name=keyV60420(legacyValueV60421(row,'name','client','client name','Client / Site Name')),address=keyV60420(legacyValueV60421(row,'address','Street Address'));
    const rows=(state.clients||[]).filter(client=>!claimed.has(client.id));
    if(idx){const hit=rows.find(client=>[client.onboardingLegacyIndexV60421,client.accountReference,client.customerReference,client.onboardingClientIdV60420].some(value=>keyV60420(value)===keyV60420(idx)));if(hit)return hit;}
    if(address){const hit=rows.find(client=>keyV60420(client.address)===address);if(hit)return hit;}
    if(name){const hits=rows.filter(client=>keyV60420(client.name)===name);if(hits.length===1)return hits[0];}
    return null;
  }
  function applyLegacyClientsV60421(model,teamMap){
    state.clients=Array.isArray(state.clients)?state.clients:[];const map=new Map(),claimed=new Set(),now=new Date().toISOString();
    model.clients.forEach((row,index)=>{
      const idx=cleanV60420(legacyValueV60421(row,'index','client id','Client ID'))||String(index+1),name=cleanV60420(legacyValueV60421(row,'name','client','client name','Client / Site Name')),address=cleanV60420(legacyValueV60421(row,'address','Street Address')),active=legacyActiveV60421(row);
      let client=legacyClientMatchStateV60421(row,claimed);
      if(!client){let id=safeImportIdV60420(`legacy-${idx}`,'client');if(state.clients.some(item=>item.id===id))id=runtimeV60423()?.uid?.('client')||`client-${Date.now()}-${index}`;client={id,siteId:`site-${id}`,createdAt:now,customerType:'Not classified',clientTypeId:'Not classified',workTypeIds:[],serviceIds:[],customTasks:'',billingNotes:'',communicationPreference:'WhatsApp',completionReport:'yes',marketingAllowed:false};state.clients.push(client);}
      const teamName=cleanV60420(legacyValueV60421(row,'team_name','team name','mobile_team','mobile team')),teamId=teamMap.get(keyV60420(teamName))||client.teamId||state.teams?.[0]?.id||'';
      const rate=moneyNumberV60420(legacyValueV60421(row,'rate','routine fee'));
      Object.assign(client,{name:name||client.name||address||`Client ${idx}`,address:address||client.address||'',suburb:cleanV60420(legacyValueV60421(row,'suburb','Suburb / Area'))||client.suburb||'',contact:cleanV60420(legacyValueV60421(row,'contact_name','contact name'))||client.contact||'',whatsapp:cleanV60420(legacyValueV60421(row,'phone','mobile'))||client.whatsapp||'',email:cleanV60420(legacyValueV60421(row,'email'))||client.email||'',serviceDescription:cleanV60420(legacyValueV60421(row,'service_scope','service scope'))||client.serviceDescription||'',gardenNotes:cleanV60420(legacyValueV60421(row,'notes'))||client.gardenNotes||'',preferredDay:legacyDayV60421(legacyValueV60421(row,'preferred_day','preferred day'))||client.preferredDay||'',teamId,preferredTeamId:teamId,status:active?'active':'archived',serviceState:active?'active':'archived',recordKindV58951:'recurring-client',activationConfirmedV58961:active,autoScheduleEnabled:false,scheduleSource:'manual-schedule-import',schedulingPolicyV58951:'manual-schedule-import',onboardingLegacyIndexV60421:idx,trialStatusV60421:cleanV60420(legacyValueV60421(row,'trial_status','trial status')),updatedAt:now,incomplete:false});
      if(Number.isFinite(rate)){client.rateAmount=rate;if(!client.monthlyFee)client.monthlyFee=0;}
      if(!client.siteId)client.siteId=`site-${client.id}`;
      claimed.add(client.id);map.set(`id:${keyV60420(idx)}`,client.id);if(name)map.set(`name:${keyV60420(name)}`,client.id);if(address)map.set(`address:${keyV60420(address)}`,client.id);
    });
    return map;
  }
  function legacyClientIdForScheduleV60421(row,map){
    const idx=cleanV60420(legacyValueV60421(row,'index','client id','Client ID')),name=cleanV60420(legacyValueV60421(row,'name','client','client name','Client / Site Name')),address=cleanV60420(legacyValueV60421(row,'address','Street Address'));
    let id=(idx&&map.get(`id:${keyV60420(idx)}`))||(address&&map.get(`address:${keyV60420(address)}`))||(name&&map.get(`name:${keyV60420(name)}`));if(id)return id;
    const existing=legacyClientMatchStateV60421(row);return existing?.id||'';
  }
  function legacyDateForScheduleV60421(row){
    const explicit=excelDateV60420(legacyValueV60421(row,'date','service date','visit date','recurrence_anchor','recurrence anchor'));if(explicit)return explicit;
    const day=legacyDayV60421(legacyValueV60421(row,'preferred_day','preferred day'));if(!day)return '';
    const today=localDateV60423(),week=runtimeV60423()?.getScheduleWeekStart?.()||startOfWeekV60423(today),dates=weekDatesV60423(week),index=Math.max(0,DAYS_V60423.indexOf(day));return dates[index]||'';
  }
  function applyLegacyScheduleV60421(model,clientMap,teamMap,fileName){
    state.schedules=Array.isArray(state.schedules)?state.schedules:[];let created=0,updated=0,preserved=0;const now=new Date().toISOString();
    model.schedules.forEach((row,index)=>{
      if(!legacyActiveV60421(row))return;const clientId=legacyClientIdForScheduleV60421(row,clientMap);if(!clientId)return;const client=(state.clients||[]).find(item=>item.id===clientId);if(!client)return;
      const date=legacyDateForScheduleV60421(row);if(!date)return;const teamRaw=cleanV60420(legacyValueV60421(row,'team_id','team id','preferred_team_id','preferred team id','team','team_name','team name')),teamId=teamMap.get(keyV60420(teamRaw))||client.teamId||state.teams?.[0]?.id||'';if(!teamId)return;
      const routeOrder=Number(legacyValueV60421(row,'route_order','route order','order','sequence'))||index+1,notes=[cleanV60420(legacyValueV60421(row,'notes')),cleanV60420(legacyValueV60421(row,'trial_status','trial status'))].filter(Boolean).join(' · '),protectedStatuses=new Set(['completed','cancelled','canceled','rescheduled','deferred','no-charge','access-failed']);
      let job=(state.schedules||[]).find(item=>item.clientId===clientId&&item.date===date&&keyV60420(item.workKind||item.workMarker)==='recurring');
      if(job&&protectedStatuses.has(keyV60420(job.status))){preserved++;return;}
      const values={date,clientId,teamId,status:'scheduled',estimatedHours:0,estimatedMinutes:0,durationUnknownV59320:true,sort:routeOrder,revenueType:'Recurring contract',workKind:'recurring',workMarker:'R',serviceIds:[...(client.serviceIds||[])],workTypeIds:[...(client.workTypeIds||[])],customTasks:client.customTasks||'',visitTasks:client.customTasks?String(client.customTasks).split(/\r?\n/).filter(Boolean):[],officeNotes:notes,autoGenerated:false,autoAssigned:false,manualOverride:true,scheduleSource:'manual-schedule-import',scheduleImportV59317:true,scheduleImportSourceV59317:{file:fileName||model.parsed?.fileName||'',row:row._row||index+2},onboardingLegacyV60421:true,updatedAt:now};
      if(job){const id=job.id,createdAt=job.createdAt,audit=job.audit;Object.assign(job,values,{id,createdAt:createdAt||now,audit});updated++;}else{job={id:runtimeV60423()?.uid?.('sch')||`sch-${Date.now()}-${index}`,createdAt:now,...values};state.schedules.push(job);created++;}
      Object.assign(client,{preferredDay:legacyDayV60421(legacyValueV60421(row,'preferred_day','preferred day'))||client.preferredDay||'',teamId,preferredTeamId:teamId,scheduleSource:'manual-schedule-import',schedulingPolicyV58951:'manual-schedule-import',autoScheduleEnabled:false,updatedAt:now});
    });
    return {created,updated,preserved};
  }
  async function commitLegacyV60421(model,fileName){
    if(model.issues.length)throw new Error('Fix the workbook issues shown before importing.');if(!workspaceWriteReadyV60420())throw new Error(workspaceWriteIssueV60420());syncStateV60423();const backup=JSON.parse(JSON.stringify(state));
    try{const teamMap=applyLegacyTeamsV60421(model),clientMap=applyLegacyClientsV60421(model,teamMap),scheduleStats=applyLegacyScheduleV60421(model,clientMap,teamMap,fileName);state.business=state.business||{};state.business.lastOnboardingImportV60421={fileName,mode:'legacy-trial',importedAt:new Date().toISOString(),clients:model.clients.length,teams:model.teams.length,schedules:model.schedules.length};runtimeV60423()?.ensureState?.();runtimeV60423()?.save?.();try{runtimeV60423()?.renderClients?.();}catch(_){}try{runtimeV60423()?.renderSchedule?.();}catch(_){}return {legacy:true,teamCount:model.teams.length||teamMap.size,clientCount:model.clients.length||clientMap.size,scheduleDefinitions:model.schedules.length,...scheduleStats};}catch(error){runtimeV60423()?.replaceState?.(backup);state=runtimeV60423()?.getState?.()||backup;throw error;}
  }
  async function commitV60420(model,fileName){
    if(model.legacy)return commitLegacyV60421(model,fileName);
    ...
    try{
      ...
      runtimeV60423()?.ensureState?.();
    }catch(error){runtimeV60423()?.replaceState?.(backup);state=runtimeV60423()?.getState?.()||backup;throw error;}
    runtimeV60423()?.save?.();
    try{runtimeV60423()?.renderClients?.();}catch(error){console.warn('[v60.4.23] clients render',error);}try{runtimeV60423()?.renderSchedule?.();}catch(error){console.warn('[v60.4.23] schedule render',error);}try{runtimeV60423()?.renderSettings?.();}catch(_){ }
  ...
  function ensureUiV60420(){
    ...
    // v60.8.0: onboarding import remains available from Clients/Business setup,
    // but Schedule V2 owns the Schedule surface completely. Do not inject an
    // onboarding-import shortcut or observer into #view-schedule.

    if(!document.getElementById('onboardingMasterDialogV60420')){
      const dialog=document.createElement('dialog');dialog.id='onboardingMasterDialogV60420';dialog.className='dialog large-dialog';dialog.innerHTML=`<div class="dialog-shell"><div class="dialog-heading"><div><span class="eyebrow">Business setup import</span><h2>Import TuinBooks onboarding workbook</h2><p>Imports the linked business snapshot or an earlier TuinBooks trial workbook. Nothing changes until you review the preview and click Import.</p></div><button type="button" class="icon-button" data-close aria-label="Close">×</button></div><div class="master-import-v60420"><div id="masterImportDropV60420" class="master-import-drop-v60420"><div><strong id="masterImportFileNameV60420">Choose the completed onboarding workbook</strong><small>.xlsx · Smart master or earlier TuinBooks trial/client/schedule workbook</small></div><div><input id="masterImportFileV60420" type="file" accept=".xlsx,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" hidden><button type="button" class="button secondary" id="chooseMasterImportV60420">Choose workbook</button></div></div><div id="masterImportPreviewV60420" class="master-import-empty-v60420">No workbook selected.</div><p class="master-import-note-v60420">Import updates matching records and keeps existing history. It never deletes old jobs, completed work, quotes or invoices.</p><div class="master-import-actions-v60420"><button type="button" class="button secondary" data-close>Cancel</button><button type="button" class="button" id="commitMasterImportV60420" disabled>Import business snapshot</button></div></div></div>`;document.body.appendChild(dialog);
  ...
  function openV60420(){ensureUiV60420();try{syncStateV60423();}catch(error){toast(error.message,'error');return;}if(!workspaceWriteReadyV60420()){toast(workspaceWriteIssueV60420(),'error');return;}currentModelV60420=null;currentFileV60420=null;document.getElementById('masterImportFileV60420').value='';document.getElementById('masterImportFileNameV60420').textContent='Choose the completed onboarding workbook';document.getElementById('masterImportPreviewV60420').className='master-import-empty-v60420';document.getElementById('masterImportPreviewV60420').innerHTML='No workbook selected.';document.getElementById('commitMasterImportV60420').disabled=true;document.getElementById('onboardingMasterDialogV60420').showModal();}
  ...
  async function commitFromUiV60420(){
    ...
    try{const result=await commitV60420(currentModelV60420,currentFileV60420?.name||'');document.getElementById('onboardingMasterDialogV60420').close();if(result.legacy)toast(`Trial data imported: ${result.clientCount} client rows, ${result.teamCount} team${result.teamCount===1?'':'s'}, ${result.created} visit${result.created===1?'':'s'} placed (${result.updated} updated).`);else toast(`Onboarding imported: ${result.clientCount} clients, ${result.teamCount} teams, ${result.scheduleDefinitions} routine patterns. ${result.created} upcoming visits created.`);}
    catch(error){console.error('[v60.4.23] onboarding import',error);toast(error?.message||'The onboarding import failed.','error');button.disabled=false;}
    finally{button.textContent='Import business snapshot';}
  }

  function installV60420(){ensureUiV60420();window.__tuinbooksOnboardingMasterImportV60420={...publicParser,modelV60420,commitV60420,build:BUILD,runtimeReady:()=>!!runtimeV60423()?.getState?.()};document.documentElement.dataset.onboardingMasterImport='v60423';}
