...
(()=>{
'use strict';
const BUILD='R19-business-control-repair';
...
const routineMarker=job=>{try{return String(runtime()?.workMarkerForJob?.(job)||job?.workMarker||'').trim().toUpperCase()==='R'}catch(_){return !job?.quoteId&&!job?.sourceQuoteId}};
const validAssignedTeam=(client,teams)=>[client?.teamId,client?.preferredTeamId].filter(Boolean).some(id=>teams.has(String(id)));
...
function collect(){
 ...
 const planningClients=clients.filter(c=>!validFrequency(c.frequency)||!validAssignedTeam(c,teams));
 ...
 const wb=s.business?.businessWorkbookV60426||{},week1=clean(wb.week1);if(/^\d{4}-\d{2}-\d{2}$/.test(week1)){
   ...
   clients.forEach(c=>{const expected=expectedVisits(c.frequency);if(expected===null)return;const count=(s.schedules||[]).filter(j=>String(j.clientId)===String(c.id)&&clean(j.date)>=week1&&clean(j.date)<=end&&routineMarker(j)&&!['cancelled','canceled'].includes(key(j.status))).length;if(count!==expected)scheduleClients.push(c);});
...
function render(force=false){
 const panel=ensurePanel();if(!panel)return;ensureStyles();
 let groups=[];
 try{groups=collect();}catch(error){
   console.warn('[business control] collect failed',error);
   panel.innerHTML='<div class="na-head"><div><span class="eyebrow">Business control</span><h2>Needs attention</h2><p>The control summary could not be calculated safely. Other Business functions remain available.</p></div><span class="na-count">!</span></div><div class="na-clear"><strong>Refresh the business data and try again.</strong></div>';
   return;
 }
 const signature=JSON.stringify(groups.map(g=>({id:g.id,count:g.count,names:g.names,detail:g.detail,label:g.label})));
 // Do not rewrite the DOM every 2.5 seconds when nothing changed. The old
 // behaviour closed open detail rows and made the panel feel unusable.
 if(!force&&signature===lastRenderSignature&&panel.dataset.rendered==='1')return;
 lastRenderSignature=signature;panel.dataset.rendered='1';
 const total=groups.reduce((n,g)=>n+g.count,0);
 panel.innerHTML=`<div class="na-head"><div><span class="eyebrow">Business control</span><h2>Needs attention</h2><p>One place for incomplete setup and data. Each item tells you exactly what to do next.</p></div><span class="na-count">${total}</span></div>${!groups.length?'<div class="na-clear"><strong>Nothing needs attention.</strong><div>The core business setup is complete.</div></div>':`<div class="na-list">${groups.map((g,i)=>`<article class="na-item"><div class="na-item-top"><div><h3>${esc(g.title)}</h3><p>${esc(g.detail)}</p></div><span class="na-pill">${g.count} item${g.count===1?'':'s'}</span></div>${g.names.length?`<details><summary>Show affected ${g.names.length===1?'record':'records'}</summary><ul class="na-names">${g.names.map(n=>`<li>${esc(n)}</li>`).join('')}</ul></details>`:''}<ol class="na-steps">${g.steps.map(step=>`<li>${esc(step)}</li>`).join('')}</ol><div class="na-actions"><button type="button" class="button secondary compact" data-na-action="${i}">${esc(g.label)}</button></div></article>`).join('')}</div>`}`;
 panel.querySelectorAll('[data-na-action]').forEach(btn=>btn.addEventListener('click',()=>{const g=groups[Number(btn.dataset.naAction)];try{g?.action?.()}catch(e){console.warn('[needs attention]',e)}}));
}
function install(){
 ensureStyles();render(true);
 document.addEventListener('click',e=>{if(e.target?.closest?.('[data-view="business"]'))setTimeout(()=>render(true),100);});
 window.addEventListener('focus',()=>setTimeout(()=>render(false),80));
 // Keep the summary current without destroying expanded rows or keyboard focus.
 setInterval(()=>{const view=document.getElementById('view-year');if(view?.classList.contains('active'))render(false);},2500);
 window.renderBusinessNeedsAttentionV6052=(force=true)=>render(force);
 window.__tuinbooksBusinessNeedsAttentionV6052={build:BUILD,render,collect,routineMarker};
}
