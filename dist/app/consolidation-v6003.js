/* TuinBooks v60.0.3 CANARY — Schedule Stage 1A hydration bind */
(()=>{
  'use strict';
  if(!window.__TUINBOOKS_CONSOLIDATION_CANARY__)return;

  const BUILD='60.0.3-schedule-stage-1a-hydration-bind';
  ...
  function requestOperationalData(){
    ...
    Promise.resolve(operation).then(()=>{
      ...
      operationalRequested=false;
      lastError=String(error?.message||error||'Operational load failed');
      console.error('[TuinBooks v60.0.3] operational hydration failed',error);
    });
  }
  ...
  function forceScheduleRender(reason='poll'){
    ...
    try{
      ...
      console.error('[TuinBooks v60.0.3] schedule render failed',error);
      return false;
    }
  }
  ...
  function installStatus(){
    ...
    document.querySelector('.tuinbooks-canary-status-v6003')?.remove();
    const el=document.createElement('div');
    el.className='tuinbooks-canary-status-v6003';
    ...
    const update=()=>{
      ...
      el.innerHTML=`<strong>v60 Schedule Stage 1A HYDRATION</strong> · ${phase} · renderer ${d.renderer} · ${d.schedules} loaded jobs · ${d.teams} teams${error}`;
    };
    update();
  ...
  function boot(){
    document.documentElement.dataset.tuinbooksCanary='v6003';
    installStatus();
    tick();
    ...
    console.info('[TuinBooks v60.0.3 canary]',{build:BUILD,productionIndexUntouched:true,hydrationBind:true});
  }
