...
export async function loadClientWorkspace(businessId) {
    const { data, error } = await supabase.rpc('tuinbooks_v2_load_client_workspace', { p_business_id: businessId });
    if (error)
        throw error;
    const accounts = (data?.accounts ?? []).map((r) => ({ id: String(r.id), businessId, name: String(r.name ?? ''), status: String(r.status ?? 'active'), contactName: String(r.contact_name ?? ''), email: String(r.email ?? ''), phone: String(r.phone ?? '') }));
    const locations = (data?.locations ?? []).map((r) => ({ id: String(r.id), businessId, accountId: String(r.customer_id), siteName: String(r.site_name ?? ''), address: String(r.address ?? ''), suburb: String(r.suburb ?? ''), accessNotes: String(r.access_notes ?? ''), instructions: String(r.instructions ?? ''), active: r.active !== false }));
    const agreements = (data?.agreements ?? []).map((r) => ({ id: String(r.id), businessId, accountId: String(r.client_id), serviceLocationId: String(r.service_site_id), status: String(r.status ?? 'draft'), startDate: r.start_date ? String(r.start_date) : null, endDate: r.end_date ? String(r.end_date) : null, frequency: String(r.frequency ?? 'weekly'), weekdays: Array.isArray(r.weekdays) ? r.weekdays.map(Number) : [], monthlyOrdinal: r.monthly_ordinal == null ? null : Number(r.monthly_ordinal), defaultTeamId: String(r.default_team_id ?? ''), estimatedMinutes: Number(r.estimated_minutes ?? 60), serviceIds: Array.isArray(r.service_ids) ? r.service_ids.map(String) : [], notes: String(r.notes ?? ''), monthlyFee: r.monthly_fee == null ? null : Number(r.monthly_fee), scheduleSeriesId: r.schedule_series_id ? String(r.schedule_series_id) : null, version: Number(r.version ?? 0), updatedAt: r.updated_at ? String(r.updated_at) : null }));
    const mappedTeams = (data?.teams ?? []).map((r) => ({ id: String(r.id), businessId, name: String(r.name ?? ''), active: r.active !== false, capacityHours: Number(r.capacity_hours ?? 8), bufferHours: Number(r.buffer_hours ?? 0) }));
