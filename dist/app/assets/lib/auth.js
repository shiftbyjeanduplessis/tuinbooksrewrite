...
function supportRow(value) { return Array.isArray(value) ? (value[0] ?? null) : (value ?? null); }
function uuid(value) { return /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(value); }
function supportCanRead(ctx) { return !!(ctx?.operational_read ?? ctx?.allow_operational_read) || !!(ctx?.financial_read ?? ctx?.allow_financial_read) || !!(ctx?.operational_edit ?? ctx?.allow_operational_edit) || !!(ctx?.financial_edit ?? ctx?.allow_financial_edit); }
export async function loadSupportAuthContext(businessId, sessionId) {
    const session = (await supabase.auth.getSession()).data.session;
    if (!session)
        return null;
    if (!uuid(businessId))
        throw new Error('The Management link is missing a valid business ID. Return to Management and open the account again.');
    if (!uuid(sessionId))
        throw new Error('The Management link is missing the exact support session ID. Return to Management and open the account again.');
    ...
    if (!data?.business_id)
        throw new Error('No active management session was found. Return to Management and open the account again.');
    if (String(data.business_id) !== businessId)
        throw new Error('The verified support session belongs to a different client account.');
    if (data.session_id && String(data.session_id) !== sessionId)
        throw new Error('The verified support session does not match this Management link.');
    if (data.status && String(data.status).toLowerCase() !== 'active')
        throw new Error('This management session is no longer active.');
    if (data.is_unexpired === false)
        throw new Error('This management session has expired.');
    if (!supportCanRead(data))
        throw new Error('This management session has no readable account access.');
