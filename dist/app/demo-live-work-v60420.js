...
(()=>{
  'use strict';
  const BUILD='60.4.20-demo-live-work';
  const PHOTO_FILES=['showroom-work-01.png','showroom-work-02.png','showroom-work-03.png','showroom-work-04.png'];
  let hooked=false;
  ...
  const photoUrl=index=>new URL(`demo-showroom-photos/${PHOTO_FILES[index%PHOTO_FILES.length]}`,location.href).href;
  ...
  function apply(){
    ...
    window.state.business=window.state.business||{};
    window.state.business.jobPhotoModeV59386='optional';
    return true;
  }
  function refresh(){
    if(!apply())return;
