/* TuinBooks v60.7.29 — Schedule UI stability guard
   DOM-only, loaded last. Does not touch schedule data or persistence. */
(()=>{
  'use strict';
  const BUILD='60.7.29-management-loader-unified';
  ...
  function ensureFloatingClose(){
    let button=document.getElementById('scheduleDetailFloatingCloseV60729');
    if(button)return button;
    button=document.createElement('button');
    button.id='scheduleDetailFloatingCloseV60729';
    button.type='button';
    button.className='tb-schedule-detail-floating-close-v60729';
  ...
  function syncDetailExit(){
    ...
    document.documentElement.style.setProperty('--tb-detail-top-v60729',`${top}px`);
    button.style.top=`${top}px`;
  }

  function installStyles(){
    if(document.getElementById('scheduleUiStabilityStylesV60729'))return;
    const style=document.createElement('style');
    style.id='scheduleUiStabilityStylesV60729';
    style.textContent=`
      #scheduleDetailPanel:not(.hidden){
        top:var(--tb-detail-top-v60729,10px)!important;
        bottom:8px!important;
        height:auto!important;
        max-height:calc(100dvh - var(--tb-detail-top-v60729,10px) - 8px)!important;
      }
      .tb-schedule-detail-floating-close-v60729{
      ...
      .tb-schedule-detail-floating-close-v60729[hidden]{display:none!important}
      #tuinbooksBuildMarkerV60729{
  ...
  function installBuildMarker(){
    ...
    document.querySelectorAll('[id^="tuinbooksBuildV"],#tuinbooksBuildMarkerV60726,#tuinbooksBuildMarkerV60727,#tuinbooksBuildMarkerV60729').forEach(node=>node.remove());
    const marker=document.createElement('div');
    marker.id='tuinbooksBuildMarkerV60729';
    document.body.appendChild(marker);
    marker.textContent='BUILD v60.7.29';
    marker.title='TuinBooks production build 60.7.29';
  }
  ...
  window.__tuinbooksProductionStabilityV60729={build:BUILD,closeScheduleDetail,removeOnboardingShortcut};
