...
(()=>{
  ...
  const BUILD='60.3.3-schedule-frozen-authority-production';
  ...
  function supportModeV6033(){
    const q=new URLSearchParams(location.search);
    return q.get('support')==='1';
  }
  function restoreCanonicalRollingV6033(){
  ...
  function rollingReadyV6033(){
    const b=window.backendV28;
    if(!b||b.mode!=='supabase')return true;
    if(b.managementOperationalLoadRequiredV59371 && !b.managementOperationalReadyV59371)return false;
    return true;
  }
  let rollingTimerV6033=null;
  function scheduleRollingV6033(reason='schedule-open',delay=500){
    clearTimeout(rollingTimerV6033);
    rollingTimerV6033=setTimeout(()=>{
      if(!rollingReadyV6033()){scheduleRollingV6033(reason,700);return;}
      try{window.scheduleRollingRefreshV58929?.(reason,180);}catch(error){console.warn('[v60.3.3] rolling refresh',error);}
    },delay);
  }
  function boot(){
    document.documentElement.dataset.tuinbooksSchedule='v6033-production';
    restoreCanonicalRollingV6033();
    ...
    if(document.body?.dataset?.app==='mobile'){
      ...
      try{renderV6010();scheduleRollingV6033('initial-load',650);}catch(error){console.error('[v6033] initial schedule render',error);}
      document.addEventListener('click',event=>{if(event.target?.closest?.('.nav-tab[data-view="schedule"]'))setTimeout(()=>{try{renderV6010();scheduleRollingV6033('schedule-open',420);}catch{}},80);},true);
      window.addEventListener('tuinbooks:runtime-ready',()=>setTimeout(()=>{try{renderV6010();loadActions(true);scheduleRollingV6033('workspace-ready',650);}catch{}},0));
    }
    installStatus();
    console.info('[TuinBooks v60.3.3 Schedule]',{build:BUILD,calendarActions:true,mobileInstructionFirst:true,recurringMovePrompt:true,supportMode:supportModeV6033(),rollingAutomation:true});
  }
