/* TuinBooks v60.4.6 — client activation runtime bridge repair
...
(()=>{
  ...
  async function activateDirectV6045(clientForm,submit){
    ...
    try{
      ...
      console.error('[v60.4.6] direct client activation failed',error);
  ...
  function saveNonActivationV6045(clientForm,submit){
    ...
    Promise.resolve(window.saveClientForm(fakeClientSubmitEventV6045(clientForm)))
      .catch(error=>{
        console.error('[v60.4.6] client save failed',error);
  ...
  function markBuild(){
    ...
    if(marker){marker.id='tuinbooksBuildV6046';marker.textContent='v60.4.6';marker.title='Client activation runtime bridge fixed · NEW R · modal/list preserved · frozen Schedule remains v60.3.5';}
  }
  ...
  function install(){
    ...
    window.__tuinbooksClientListModalV6046={build:BUILD,listOnlyPage:true,recordSeparateModal:true,paginationRemoved:true,statusDetail:true,directActivatePath:true,frozenScheduleUntouched:true};
  }
