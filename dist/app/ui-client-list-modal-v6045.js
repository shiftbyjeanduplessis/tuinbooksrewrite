/* TuinBooks v60.4.5 — deterministic client activation repair
...
(()=>{
  'use strict';
  const BUILD='60.4.5-client-activate-direct';
  ...
  function ensureNewRBasketV6045(client){
    ...
    if(typeof addNewRecurringClientToBasketV6036!=='function')throw new Error('NEW R basket function is unavailable.');
    const item=addNewRecurringClientToBasketV6036(client);
  ...
  async function activateDirectV6045(clientForm,submit){
    ...
    try{
      ...
      status.value='paused';
      const result=await Promise.resolve(saveClientForm(fakeClientSubmitEventV6045(clientForm)));
      ...
      console.error('[v60.4.5] direct client activation failed',error);
  ...
  function saveNonActivationV6045(clientForm,submit){
    ...
    Promise.resolve(saveClientForm(fakeClientSubmitEventV6045(clientForm)))
      .catch(error=>{
        console.error('[v60.4.5] client save failed',error);
  ...
  function markBuild(){
    ...
    if(marker){marker.id='tuinbooksBuildV6045';marker.textContent='v60.4.5';marker.title='Deterministic client activation · NEW R · modal/list preserved · frozen Schedule remains v60.3.5';}
  }
  ...
  function install(){
    ...
    installStyles();ensureBackdrop();addNewClientButton();addModalHint();bindClose();bindArchiveClose();bindEscape();bindClientNav();observeList();bindReliableSubmitV6045();
    ...
    window.__tuinbooksClientListModalV6045={build:BUILD,listOnlyPage:true,recordSeparateModal:true,paginationRemoved:true,statusDetail:true,directActivatePath:true,frozenScheduleUntouched:true};
  }
