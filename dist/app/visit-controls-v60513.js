(() => {
  ...
  const BUILD = '60.5.13-visit-ui-confirmed-work-single-adhoc';
  ...
  function reconcileScheduleActionRenderersV60511(){
    ...
    const paintCanonical=()=>{
      try{
        document.querySelectorAll('#weeklyScheduleBoard .schedule-day-lane[data-team-id][data-date]').forEach(lane=>{
          // Keep the original v60.0.5 card the user approved. The later v60.1.0
          // renderer is the duplicate that was replacing it after board repaints.
          lane.querySelectorAll(':scope .schedule-actions-v6010').forEach(node=>node.remove());
          if(lane.querySelector(':scope .schedule-actions-v6005, :scope .schedule-lane-cards .schedule-actions-v6005'))return;
          if(typeof window.scheduleActionsForV6005!=='function' || typeof window.scheduleActionHtmlV6005!=='function')return;
          const rows=window.scheduleActionsForV6005(lane.dataset.teamId||'',lane.dataset.date||'')||[];
          if(!rows.length)return;
          const past=typeof window.scheduleIsPast==='function'?window.scheduleIsPast(lane.dataset.date||''):false;
          const host=document.createElement('div');
          host.className='schedule-actions-v6005';
          host.innerHTML=rows.map(action=>window.scheduleActionHtmlV6005(action,past)).join('');
          const cards=lane.querySelector(':scope > .schedule-lane-cards');
          if(cards)cards.insertBefore(host,cards.firstChild);
          else{
            const jobs=lane.querySelector(':scope > .v6006-jobs');
            if(jobs)lane.insertBefore(host,jobs);else lane.appendChild(host);
          }
        });
      }catch(_){ }
    };
    // Remove the duplicate renderer immediately so it can never become the visible card.
    try{document.querySelectorAll('#weeklyScheduleBoard .schedule-actions-v6010').forEach(node=>node.remove());}catch(_){ }
    paintCanonical();
    // v6005 actions load asynchronously on first workspace paint. Repaint the approved
    // version once that canonical data is available, without rebuilding the schedule.
    try{
      if(typeof window.loadScheduleActionsV6005==='function')
        Promise.resolve(window.loadScheduleActionsV6005(false)).then(()=>requestAnimationFrame(paintCanonical)).catch(()=>{});
    }catch(_){ }
  }
  function injectStyles() {
    ...
    style.textContent = `
      ...
      /* One ad-hoc renderer only: keep the original v6005 card the user approved. */
      #weeklyScheduleBoard .schedule-actions-v6010{display:none!important}
      #weeklyScheduleBoard .schedule-actions-v6005{display:grid!important}
    ...
    if(document.body?.dataset?.app!=='mobile'){
      ...
      if(host){
        ...
        const observer=new MutationObserver(()=>{
          ...
          requestAnimationFrame(()=>{queued=false;reconcileScheduleActionRenderersV60511();decorateScheduleCards();});
        });
  ...
  window.__tuinbooksSchedulePolishV60511Test={explicitCadence,inferCadence,clientPattern,targetForWeek,rollingWorkspaceReadyV60511,maintainRollingScheduleV60511,cloudOccurrenceIndexV60511,cloudBlocksOccurrenceV60511,installSingleRollingAuthorityV60511,explicitClientTasksV60511,reconcileScheduleActionRenderersV60511,clientWorkConfirmedV60512,confirmedServiceIdsV60512,confirmedTaskLinesV60512,replaceWorkRequiredV60512};
