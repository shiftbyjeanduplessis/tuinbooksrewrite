...
(()=>{
  ...
  const BUILD='60.0.10-schedule-actions-mobile-note';
  ...
  async function loadActions(force=false){
    ...
    actionsPromise=(async()=>{
      try{
        ...
        console.warn('[TuinBooks v60.0.10] action load failed',error);
        return false;
  ...
  async function persistAction(action){
    ...
    if(result.error){
      ...
      if(/function|rpc|schema cache|not found/i.test(message)){
        const direct={
          id:row.id,business_id:b.businessId,title:row.title,detail:row.detail,status:row.status||'scheduled',priority:'normal',
          assigned_type:'team',assigned_id:row.assignedId||'',calendar_date:row.calendarDate,calendar_time:row.calendarTime||null,payload
        };
        result=await b.client.from('operational_actions_v59674').upsert(direct,{onConflict:'id'}).select('*').single();
      }
  ...
  function applyFuturePattern(job){
    ...
    saveState();try{window.renderSchedule?.();}catch{}
  ...
  function renderV6010(){
    ...
    patchCalendarButtons();injectDesktopActions();loadActions(false);document.documentElement.dataset.scheduleRenderer='v6010';return result;
  }
  ...
  function installStatus(){
    document.querySelectorAll('[class^="tuinbooks-canary-status-v60"]').forEach(n=>n.remove());
    const el=document.createElement('div');el.className='tuinbooks-canary-status-v6010';document.body.appendChild(el);
    const update=()=>{
      const st=appState(),controls=document.querySelectorAll('.v6010-calendar-actions').length,mobile=document.body?.dataset?.app==='mobile';
      const mobileFirst=mobile?Boolean(document.querySelector('#mobileScheduleList > .mobile-actions-v6010')):false;
      el.innerHTML=`<strong>v60.0.10 ACTIONS + MOBILE NOTE</strong> · ${location.pathname.endsWith('consolidation-v6010.html')||location.pathname.endsWith('mobile-v6010.html')?'unique page YES':'WRONG PAGE'} · ${(st.schedules||[]).length} jobs · ${(st.teams||[]).length} teams · ${actions.length} saved actions · ${mobile?`mobile first-item ${mobileFirst?'YES':'waiting'}`:`${controls} team-day action sets`} · production untouched`;
    };
    update();setInterval(update,900);
  }

  function boot(){
    document.documentElement.dataset.tuinbooksCanary='v6010';
    ...
    if(document.body?.dataset?.app==='mobile'){
      ...
      try{renderV6010();}catch(error){console.error('[v6010] initial schedule render',error);}
      document.addEventListener('click',event=>{if(event.target?.closest?.('.nav-tab[data-view="schedule"]'))setTimeout(()=>{try{renderV6010();}catch{}},80);},true);
      window.addEventListener('tuinbooks:runtime-ready',()=>setTimeout(()=>{try{renderV6010();loadActions(true);}catch{}},0));
    }
    installStatus();
    console.info('[TuinBooks v60.0.10 canary]',{build:BUILD,calendarActions:true,mobileInstructionFirst:true,recurringMovePrompt:true,productionUntouched:true});
  }
