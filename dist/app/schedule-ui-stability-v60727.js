/* TuinBooks v60.7.27 — Schedule UI stability guard
   DOM-only, loaded last. Does not touch schedule data or persistence. */
(()=>{
  'use strict';
  const BUILD='60.7.27-management-loading-stability';
  ...
  window.closeScheduleQueue=function closeScheduleQueueUiV60726(){
    try{closeBasketBeforeV60726?.();}catch(error){console.warn('[TuinBooks v60.7.27] basket close fallback',error);}
    closeBasketToTag();
  };
  ...
  function ensureFloatingClose(){
    let button=document.getElementById('scheduleDetailFloatingCloseV60727');
    if(button)return button;
    button=document.createElement('button');
    button.id='scheduleDetailFloatingCloseV60727';
    button.type='button';
    button.className='tb-schedule-detail-floating-close-v60727';
  ...
  function syncDetailExit(){
    ...
    document.documentElement.style.setProperty('--tb-detail-top-v60727',`${top}px`);
    button.style.top=`${top}px`;
  }

  function installStyles(){
    if(document.getElementById('scheduleUiStabilityStylesV60727'))return;
    const style=document.createElement('style');
    style.id='scheduleUiStabilityStylesV60727';
    style.textContent=`
      #scheduleDetailPanel:not(.hidden){
        top:var(--tb-detail-top-v60727,10px)!important;
        bottom:8px!important;
        height:auto!important;
        max-height:calc(100dvh - var(--tb-detail-top-v60727,10px) - 8px)!important;
      }
      .tb-schedule-detail-floating-close-v60727{
      ...
      .tb-schedule-detail-floating-close-v60727[hidden]{display:none!important}
      #tuinbooksBuildMarkerV60727{
  ...
  function installBuildMarker(){
    ...
    document.querySelectorAll('[id^="tuinbooksBuildV"],#tuinbooksBuildMarkerV60726,#tuinbooksBuildMarkerV60727').forEach(node=>node.remove());
    const marker=document.createElement('div');
    marker.id='tuinbooksBuildMarkerV60727';
    document.body.appendChild(marker);
    marker.textContent='BUILD v60.7.27';
    marker.title='TuinBooks production build 60.7.27';
  }
  ...
  window.__tuinbooksProductionStabilityV60727={build:BUILD,closeScheduleDetail,removeOnboardingShortcut};
