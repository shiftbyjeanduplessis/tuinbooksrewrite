...
(()=>{
  'use strict';
  const BUILD='60.4.13-demo-native-layout-guide';
  ...
  const TOUR_STORAGE_PREFIX='tuinbooks-demo-tour-v60413:';
  let tourIndex=-1;
  ...
  const GUIDE_STEPS=[
    {view:'schedule',label:'Schedule',title:'Run the working week',copy:'This is the normal TuinBooks Schedule. Two teams have a realistic week loaded. Today is deliberately mid-shift, so some visits are complete and others are still on the road. Open jobs, change days and explore normally.'},
    {view:'records',label:'Work',title:'See live progress from the teams',copy:'This is the normal Work screen. The built-in Today by team cards show completion percentages. Completed jobs carry photo evidence; I open one example so you can see the same detail the office receives from the field phone.',openWorkExample:true},
    {view:'clients',label:'Clients',title:'Open a client record',copy:'The demo has residential, estate and commercial clients with recurrence, service instructions, rates and billing details. Nothing here is real customer information.'},
    {view:'quotes',label:'Quotes',title:'Follow a quote through the pipeline',copy:'You can inspect Draft, Sent, Accepted and Completed examples. Accepted work can flow back into scheduling without creating a separate system.'},
    {view:'invoices',label:'Billing',title:'See work reach billing',copy:'These are the normal TuinBooks Billing screens with routine drafts, quoted work ready to invoice, and examples of sent and paid invoices. Demo outbound sending stays disabled.'},
    {view:null,label:'Field phone',title:'Now look at the bakkie side',copy:'The field phone shows a team only what it needs for the day. Open it in another tab, enter the demo PIN once, then open a job, tick off work, add a note or photo and complete the visit.',mobile:true}
  ];
  ...
  function guideFooter(){
    return `<div class="tb-demo-mobile-strip-v60413"><div><span>Field phone</span><strong>PIN ${esc(DEMO_FIELD_PIN)}</strong></div><button type="button" data-demo-open-mobile-v60413>Open mobile</button></div>`;
  }
  ...
  function showGuideHome(){
    ...
    card.innerHTML=`<button type="button" class="tb-demo-guide-close-v60413" data-demo-close-v60413 aria-label="Close demo guide">×</button><span class="tb-demo-guide-kicker-v60413">TuinBooks demo</span><h3>Explore the actual software</h3><p>This is a populated training business running on the normal TuinBooks screens. The guide never locks the page — close it at any time and click around freely.</p><div class="tb-demo-guide-facts-v60413"><span><b>2</b> teams</span><span><b>30</b> clients</span><span><b>8</b> jobs today</span></div>${guideFooter()}<div class="tb-demo-guide-actions-v60413"><button type="button" class="secondary" data-demo-explore-v60413>Explore freely</button><button type="button" data-demo-start-v60413>Start guided tour</button></div>`;
  ...
  async function openNativeWorkExample(){
    ...
    await sleep(180);
    const completed=[...document.querySelectorAll('#view-records details.work-job-card-v59386')].find(node=>node.classList.contains('completed'))||document.querySelector('#view-records details.work-job-card-v59386');
    if(completed){
      completed.open=true;
  ...
  async function renderGuideStep(index){
    ...
    card.innerHTML=`<button type="button" class="tb-demo-guide-close-v60413" data-demo-close-v60413 aria-label="Close demo guide">×</button><span class="tb-demo-guide-kicker-v60413">Step ${guideIndex+1} of ${GUIDE_STEPS.length} · ${esc(step.label)}</span><h3>${esc(step.title)}</h3><p>${esc(step.copy)}</p>${guideFooter()}<div class="tb-demo-guide-progress-v60413"><i style="width:${Math.round((guideIndex+1)/GUIDE_STEPS.length*100)}%"></i></div><div class="tb-demo-guide-actions-v60413"><button type="button" class="secondary" data-demo-exit-v60413>Exit tour</button><div>${guideIndex?'<button type="button" class="secondary" data-demo-back-v60413>Back</button>':''}<button type="button" data-demo-next-v60413>${last?'Finish':`Next: ${esc(GUIDE_STEPS[guideIndex+1].label)}`}</button></div></div>`;
    bindCommonGuideButtons(card);
  ...
  function finishGuide(){
    try{localStorage.setItem(`${TOUR_STORAGE_PREFIX}${backend()?.businessId||'demo'}`,'done');}catch(_){ }
    closeGuide();
  ...
  function installDemoUI(){
    ...
    let done=false;try{done=localStorage.getItem(key)==='done';}catch(_){ }
    if(!done&&!guideCard)setTimeout(showGuideHome,350);
  }
  ...
  document.addEventListener('click',event=>{
    ...
    if(nav&&isDemo())setTimeout(()=>{applyDemoPhotosToState();try{window.renderRecords?.();}catch(_){ }},180);
  });

  window.installTuinBooksDemoShowroomV60413=restoreShowroom;
  window.__tuinbooksDemoShowroomV60413={build:BUILD,restore:restoreShowroom,startTour:()=>renderGuideStep(0),buildState:buildShowroomState,isDemo};
