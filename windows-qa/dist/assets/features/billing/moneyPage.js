...
import { createInvoiceFromFacts, loadMoneyWorkspace, recordPayment, reversePayment, saveInvoice, saveQuote, scheduleAcceptedQuote, setInvoiceStatus, setQuoteStatus, setVisitBillingAmount } from './moneyRepository.js';
...
export function renderMoneyPage(root, identity, navigation, mode = 'money') {
    ...
    page.innerHTML = `<section class="workspace-heading"><div><p class="eyebrow">${mode === 'quotes' ? 'Sales' : 'Billing'}</p><h1>${mode === 'quotes' ? 'Quotes' : 'Money'}</h1><p>${mode === 'quotes' ? 'Quotes remain separate from operational Additional Visits.' : 'Billing reads completed/cancelled visit facts; it does not infer work from UI state.'}</p></div><div class="toolbar-actions"><button id="moneyRefresh">Refresh</button>${mode === 'quotes' ? '<button id="newQuote" class="primary-button">+ Quote</button>' : '<button id="newInvoice" class="primary-button">+ Invoice</button>'}</div></section><div id="moneyContent"><div class="loading-state">Loading…</div></div>`;
    ...
    page.querySelector('#newInvoice')?.addEventListener('click', () => data && openInvoiceDialog(data.accounts, null, data.defaultVatRate, input => mutate(() => saveInvoice(identity.businessId, input))));
    ...
    function drawStatements(host) { if (!data)
        return; host.innerHTML = `<label class="statement-select">Client<select id="statementAccount"><option value="">Choose client</option>${data.accounts.map(a => `<option value="${attr(a.id)}">${esc(a.name)}</option>`).join('')}</select></label><div class="statement-actions"><button id="emailStatement">Email statement</button><button id="waStatement">WhatsApp statement</button></div><div id="statementRows" class="statement-rows"><div class="empty-state">Choose a client.</div></div>`; const select = host.querySelector('#statementAccount'), rows = host.querySelector('#statementRows'); host.querySelector('#emailStatement').onclick = () => deliverStatement(select.value, 'email'); host.querySelector('#waStatement').onclick = () => deliverStatement(select.value, 'whatsapp'); select.onchange = () => { const list = statementRows(select.value, data.invoices, data.payments); rows.innerHTML = list.map(r => `<div><span>${r.date}</span><span>${r.type} ${esc(r.reference)}</span><span>${r.debit ? `R ${r.debit.toFixed(2)}` : ''}</span><span>${r.credit ? `R ${r.credit.toFixed(2)}` : ''}</span><strong>R ${r.balance.toFixed(2)}</strong></div>`).join('') || '<div class="empty-state">No statement activity.</div>'; }; }
    function deliverQuote(q, channel) { if (!data)
        return; const account = data.accounts.find(a => a.id === q.accountId); if (!account)
        return; const text = `${identity.businessName}\nQuote ${q.number}\nTotal: R ${quoteTotal(q).toFixed(2)}\nValid until: ${q.validUntil || 'Not set'}\n${q.lines.map(l => `${l.description} · R ${documentTotal([l]).toFixed(2)}`).join('\n')}`; if (channel === 'email') {
        if (!account.email) {
    ...
    function deliverInvoice(inv, channel) { if (!data)
        return; const account = data.accounts.find(a => a.id === inv.accountId); if (!account)
        return; const text = `${identity.businessName}\nInvoice ${inv.number}\nTotal: R ${documentTotal(inv.lines).toFixed(2)}\nOutstanding: R ${invoiceBalance(inv, data.payments).toFixed(2)}\nDue: ${inv.dueDate || 'Not set'}`; if (channel === 'email') {
        if (!account.email) {
            alert('This client has no email address.');
            return;
        }
    ...
    function deliverStatement(accountId, channel) { if (!data || !accountId) {
        alert('Choose a client first.');
        return;
    } const account = data.accounts.find(a => a.id === accountId); if (!account)
        return; const rows = statementRows(accountId, data.invoices, data.payments), balance = rows.at(-1)?.balance ?? 0, text = `${identity.businessName}\nStatement for ${account.name}\nCurrent balance: R ${balance.toFixed(2)}\n${rows.map(r => `${r.date} ${r.type} ${r.reference} · Balance R ${r.balance.toFixed(2)}`).join('\n')}`; if (channel === 'email') {
        if (!account.email) {
            alert('This client has no email address.');
            return;
        }
