...
import { loadAdditionalVisitClients, loadWeek, persistAdditionalVisit, persistCancelVisit, persistClientHold, persistDayAction, persistMarkMissed, persistQueuePlacement, persistRemoveDayAction, persistRescheduleMissed, persistSuspension, persistUndoCancel, persistVisitMove, persistVisitResize, persistVisitToBasket } from './scheduleRepository.js';
...
export function renderSchedulePage(root, identity, navigation) {
    ...
    async function openAdditional(date, teamId, _index, sortOrder) { if (!data)
        return; let choices = { accounts: data.accounts, locations: data.locations }; if (!identity.demo) {
        try {
            choices = await loadAdditionalVisitClients(identity.businessId);
        }
        catch (error) {
            showError(`Could not load clients for Additional Visit. ${msg(error)}`);
            return;
        }
    } openAdditionalVisitDialog({ businessId: identity.businessId, date, teamId, sortOrder, ...choices }, createAdditional); }
