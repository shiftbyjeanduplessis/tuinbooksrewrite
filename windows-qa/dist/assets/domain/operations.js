...
export function upsertDayActionOptimistically(actions, businessId, input) {
    const row = { id: input.id, businessId, date: input.date, teamId: input.teamId, kind: input.kind, title: input.kind === 'team_note' ? 'Day instruction' : input.title.trim(), detail: input.detail.trim(), time: input.kind === 'team_note' ? '' : input.time, status: 'active', updatedAt: null };
