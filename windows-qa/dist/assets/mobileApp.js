...
} const pin = root.querySelector('#mobilePin').value.trim(); const result = await supabase.rpc('claim_mobile_access_code', { p_code: pin, p_device_name: 'TuinBooks v2 field phone' }); if (result.error) {
    show(result.error.message);
    return;
