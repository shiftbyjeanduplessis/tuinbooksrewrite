...
-- Reuses public.quotes/public.invoices; adds relational lines/payments and explicit RPCs.
begin;
...
grant select on public.quote_lines_v2,public.invoice_lines_v2,public.payments_v2 to authenticated;
revoke insert,update,delete on public.quote_lines_v2,public.invoice_lines_v2,public.payments_v2 from authenticated;
...
-- Resolve the amount a routine occurrence would normally carry. This is used
-- for chargeable cancellations only; Additional visits remain a Billing decision.
...
declare
 v_accounts jsonb;v_quotes jsonb;v_invoices jsonb;v_payments jsonb;v_facts jsonb;v_settings jsonb;
begin
 ...
 select coalesce(jsonb_agg(x order by x->>'visit_date'),'[]') into v_facts from(
   select jsonb_build_object(
    ...
    'already_invoiced',exists(select 1 from public.invoice_lines_v2 l where l.business_id=j.business_id and l.source_visit_id=j.id)
   ) x
...
returns text language plpgsql security definer set search_path=public as $$
declare q public.quotes%rowtype;qid text:='queue-quote-'||p_quote_id;desc_text text;total numeric;
begin
 if not public.is_business_admin(p_business_id) then raise exception 'Admin access required';end if;
 ...
 select string_agg(description,', '),sum(quantity*unit_price*(1+vat_rate/100)) into desc_text,total from public.quote_lines_v2 where business_id=p_business_id and quote_id=p_quote_id;
 ...
 values(p_business_id,qid,q.client_id,0,'quoted','quoted','Accepted quote','open',jsonb_build_object('sourceQuoteId',p_quote_id,'task',coalesce(desc_text,'Quoted work'),'quotedTotal',coalesce(total,0),'visitType','quoted','billingDisposition','quoted'),auth.uid(),auth.uid())
...
begin
 ...
 select lower(coalesce(c.payload#>>'{v2Billing,billingBasis}','')),case when coalesce(c.payload#>>'{v2Billing,billingAmount}','')~'^-?[0-9]+([.][0-9]+)?$' then (c.payload#>>'{v2Billing,billingAmount}')::numeric else null end
 into billing_basis,monthly_fee from public.customers c where c.business_id=p_business_id and c.id=p_client_id;
 if monthly_fee is null then select sum(a.monthly_fee) into monthly_fee from public.client_service_agreements_v2 a where a.business_id=p_business_id and a.client_id=p_client_id and a.status='active' and a.monthly_fee is not null;end if;
 monthly_mode:=billing_basis like '%month%' or (billing_basis='' and monthly_fee is not null);

 for v in
   select j.*,
    case when lower(coalesce(j.payload->>'visitType','')) like '%additional%' then 'additional' when lower(coalesce(j.payload->>'visitType',j.payload->>'revenueType','')) like '%quote%' then 'quoted' else 'routine' end vt,
 ...
 loop
   ...
   -- A monthly routine cancellation marked "charge" still belongs to the normal
   -- monthly service fee. It must not become a second monthly-fee line.
   if v.vt='routine' and (lower(v.status)='completed' or (lower(v.status)='cancelled' and v.bd='charge' and monthly_mode)) then
     if not routine_added then
       fee:=monthly_fee;
       if fee is null then fee:=public.tuinbooks_v2_visit_default_amount(p_business_id,p_client_id,v.payload);end if;
     ...
     if coalesce(v.payload->>'quotedTotal','')~'^[0-9]+([.][0-9]+)?$' then fee=(v.payload->>'quotedTotal')::numeric;
     ...
     lines=lines||jsonb_build_array(jsonb_build_object('id','line-'||v.id,'description',coalesce(nullif(v.payload->>'task',''),'Quoted work'),'quantity',1,'unitPrice',fee,'vatRate',vat,'sourceVisitId',v.id,'sourceQuoteId',nullif(v.payload->>'sourceQuoteId',''),'category','quoted'));
     ...
     -- Additional visits deliberately do not inherit the routine fee. Billing
     -- chooses the amount after the work is known.
...
revoke all on function public.tuinbooks_v2_money_lines_json(uuid,text,text,jsonb) from public,anon;
revoke all on function public.tuinbooks_v2_visit_default_amount(uuid,text,jsonb) from public,anon;
