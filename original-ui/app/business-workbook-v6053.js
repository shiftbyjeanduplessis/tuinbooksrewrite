...
(()=>{
'use strict';
const BUILD='60.5.3-business-workbook-ordered-cloud-save';
...
function toastMsg(m,t=''){const el=document.getElementById('bwMessage');if(el){el.textContent=String(m||'');el.className=`message ${t==='error'?'error':t==='success'?'success':''}`;}try{hostWindow()?.toast?.(m,t)}catch(_){}}
...
function normStatus(v){const k=key(v);return k==='archived'?'archived':k==='suspended'?'suspended':'active';}
...
function sleepV6053(ms){return new Promise(resolve=>setTimeout(resolve,ms));}
async function waitCloudIdleV6053(backend,timeoutMs=30000){
 const started=Date.now();
 while((backend?.syncing||backend?.operationalSyncing)&&Date.now()-started<timeoutMs)await sleepV6053(100);
 if(backend?.syncing||backend?.operationalSyncing)throw new Error('TuinBooks is still finishing an earlier cloud save. Wait a few seconds and import again.');
 return true;
}
function stopQueuedCloudTimersV6053(backend){
...
function isTimeoutV6053(error){return /timeout|timed out|statement timeout|57014|canceling statement/i.test(String(error?.message||error||''));}
async function rpcWithOneTimeoutRetryV6053(backend,name,args){
 let response=await backend.client.rpc(name,args);
 if(response?.error&&isTimeoutV6053(response.error)){await sleepV6053(1200);response=await backend.client.rpc(name,args);}
 return response;
}
async function persistImportOrderedV6053(rt){
 const backend=rt?.getBackend?.();
 if(!backend||backend.mode!=='supabase'||!backend.client||!backend.businessId){rt?.save?.();return true;}
 await waitCloudIdleV6053(backend);
 stopQueuedCloudTimersV6053(backend);
 // Let the application's own save builders create the exact core and operational snapshots.
 // The timers are then cancelled and the two snapshots are persisted in FK-safe order.
 rt?.save?.();
 stopQueuedCloudTimersV6053(backend);
 const corePending=backend.pendingSnapshot||null,opPending=backend.operationalPendingSnapshot||null;
 if(corePending?.snapshot){
   const core=await rpcWithOneTimeoutRetryV6053(backend,'save_core_snapshot_v53',corePending.snapshot);
   if(core.error)throw core.error;
   backend.coreRevision=Number(core.data?.revision??backend.coreRevision+1);
   backend.lastSnapshotJson=corePending.json||'';backend.pendingSnapshot=null;backend.coreDirty=false;backend.coreConflict=false;
 }
 if(opPending?.snapshot){
   // Core MUST finish first: schedule_jobs has a composite FK to teams.
   opPending.snapshot.p_expected_revision=Number(backend.operationalRevision||opPending.snapshot.p_expected_revision||0);
   let op=await rpcWithOneTimeoutRetryV6053(backend,'save_operational_snapshot_v5604',opPending.snapshot);
   if(op.error&&/Could not find the function|schema cache|PGRST202/i.test(String(op.error?.message||op.error)))op=await rpcWithOneTimeoutRetryV6053(backend,'save_operational_snapshot_v53',opPending.snapshot);
   if(op.error){try{op.error.tuinbooksCoreSavedV6053=Boolean(corePending?.snapshot);}catch(_){}throw op.error;}
   backend.operationalRevision=Number(op.data?.revision??backend.operationalRevision+1);
   backend.lastOperationalJson=opPending.json||'';backend.operationalPendingSnapshot=null;backend.operationalDirty=false;backend.operationalConflict=false;backend.lastOperationalErrorV5604=null;
 }
 return true;
}
async function ensureRollingWithoutCloudRaceV6053(rt){
 ...
 try{await ensure({reason:'business-workbook-import-v6053',showFeedback:false});}catch(error){console.warn('[v60.5.3 workbook] rolling extension after import',error);}
}
...
async function apply(model,options={}){
 ...
 try{
   ...
   try{const media=model.parsed._media||[];if(media[0]){s.business.logoDataUrlV60432=media[0];s.business.logoDataUrl=media[0];}}catch(_){ }
   ...
   s.teams=Array.isArray(s.teams)?s.teams:[];const teamByWorkbookId=new Map(),teamByName=new Map();
   model.teams.forEach(r=>{const name=clean(get(r,'Team Name')),hidden=clean(get(r,'TuinBooks ID'));let t=hidden?s.teams.find(x=>String(x.id)===hidden):s.teams.find(x=>key(x.name)===key(name));if(!t){t={id:uid('team'),capacityHours:8,bufferHours:0,dailySiteCapacity:0,createdAt:new Date().toISOString()};s.teams.push(t);}t.name=name;t.active=true;t.updatedAt=new Date().toISOString();teamByName.set(key(name),t.id);if(hidden)teamByWorkbookId.set(hidden,t.id);});
   ...
   if(model.financial){for(let i=0;i<model.billProfiles.length;i++){
     ...
     const saved=await upsertProfile(hidden||existing?.id||null,payload);profileByName.set(key(name),saved.id);if(hidden)profileByWorkbookId.set(hidden,saved.id);
   }}
   ...
   if(replaceClients){
     ...
     s.schedules=s.schedules.filter(j=>{if(!oldClientIds.has(String(j.clientId)))return true;const d=clean(j.date),st=key(j.status);return d<todayIso||historicalStatus.has(st);});
   ...
   model.schedule.forEach((r,idx)=>{const cid=resolveClientId(r),tid=resolveTeamId(r),week=Number(get(r,'Week')),day=clean(get(r,'Day'));if(!cid||!tid||![1,2,3,4].includes(week)||!DAYS.includes(day))return;const date=addDays(model.week1,(week-1)*7+DAYS.indexOf(day)),jobHidden=clean(get(r,'TuinBooks Schedule ID'))||clean(get(r,'TuinBooks ID'));let job=jobHidden?s.schedules.find(x=>String(x.id)===jobHidden):s.schedules.find(x=>x.clientId===cid&&x.date===date&&(x.workKind==='recurring'||x.workMarker==='R')&&!['cancelled','canceled'].includes(key(x.status)));if(!job){job={id:uid('sch'),createdAt:new Date().toISOString()};s.schedules.push(job);}Object.assign(job,{date,clientId:cid,teamId:tid,status:'scheduled',sort:idx+1,workKind:'recurring',workMarker:'R',revenueType:'Recurring contract',serviceIds:[...(s.clients.find(c=>c.id===cid)?.serviceIds||[])],officeNotes:clean(get(r,'Notes')),startTimeV60426:clean(get(r,'Start Time')),autoGenerated:false,manualOverride:true,onboardingMasterV60426:true,updatedAt:new Date().toISOString()});if(!clientFirst.has(cid)||date<clientFirst.get(cid))clientFirst.set(cid,date);});
   ...
   rt?.ensureState?.();
   // Build recurring agreement metadata before taking the one operational snapshot.
   try{if(typeof hostWindow()?.ensureRecurringAgreementCoverageV5608==='function')hostWindow().ensureRecurringAgreementCoverageV5608();}catch(_){}
   // v60.5.3: persist core first, then operational. Previously rt.save() queued operational
   // before core (700ms vs 850ms), and the rolling reconciler also forced another operational
   // save immediately. A new schedule job could therefore reference a team that was not yet
   // committed in Supabase. The import now performs one ordered cloud commit.
   await persistImportOrderedV6053(rt);
   // Do not run rolling reconciliation inside the import. The imported workbook already supplies
   // the standard four-week schedule; normal rolling maintenance can extend it after the import.
   try{rt?.renderClients?.();}catch(_){}try{rt?.renderSchedule?.();}catch(_){}try{rt?.renderSettings?.();}catch(_){}try{hostWindow()?.renderBusinessNeedsAttentionV6052?.();}catch(_){}
   return {clients:model.clients.length,teams:model.teams.length,visits:model.schedule.length,attention:(model.attention||[]).length,replaced:replaceClients?existingClients.length:0};
 }catch(error){
   // Do not launch another pair of competing cloud snapshots while reporting an import failure.
   // If core already committed, keep the in-memory import state so a retry can finish the
   // operational save against the now-existing teams instead of silently diverging from Supabase.
   try{
     const backend=rt?.getBackend?.();stopQueuedCloudTimersV6053(backend);
     if(!error?.tuinbooksCoreSavedV6053)rt?.replaceState?.(backup);
     if(backend){const h=backend.hydrating,oh=backend.operationalHydrating;backend.hydrating=true;backend.operationalHydrating=true;try{rt?.save?.();}finally{backend.hydrating=h;backend.operationalHydrating=oh;}}
     else rt?.save?.();
   }catch(_){}
   throw error;
 }
...
function exportData(){
 ...
 const teams=(s.teams||[]).filter(x=>x.active!==false),start=currentMonday(),end=addDays(start,27),scheduleRows=[];
...
async function runImport(){
 ...
 if(replaceClients){const ok=window.confirm(`Replace the current client setup for ${hostBusinessName()}?\n\nThe workbook will become the current client list. Existing clients that are not in the workbook will be removed, and current client service/billing setup plus future unresolved scheduling will be rebuilt from the workbook. Completed work and issued documents remain as history.`);if(!ok)return;}
 const b=document.getElementById('bwCommit');if(b){b.disabled=true;b.textContent=replaceClients?'Replacing client data…':'Importing…';}
 try{const r=await apply(current,{replaceClients});const extra=r.attention?` ${r.attention} item${r.attention===1?'':'s'} are now listed under Business → Needs attention.`:'';toastMsg(`${replaceClients?'Replaced client setup and imported':'Imported'} ${r.clients} clients, ${r.teams} teams and ${r.visits} standard visits.${extra}`,'success');current=null;currentFile=null;const f=document.getElementById('bwFile');if(f)f.value='';const p=document.getElementById('bwPreview');if(p)p.innerHTML=`<div class="empty-preview"><strong>Import complete.</strong>${r.attention?` ${r.attention} item${r.attention===1?'':'s'} can be worked through under <strong>Business → Needs attention</strong>.`:''}</div>`;if(b)b.disabled=true;}catch(e){console.error('[v60.5.2 import]',e);toastMsg(e.message||String(e),'error');if(b)b.disabled=false;}finally{if(b)b.textContent='Import business';}
}
function install(){
 window.__tuinbooksBusinessWorkbookV6052={build:BUILD,model,apply,exportWorkbook,buildExportBlob,downloadBlank,hostReady};
 const week=document.getElementById('bwWeek1');if(week&&!week.value)week.value=mondayThisWeek();
