(() => {
  ...
  const BUILD = '60.5.18-approved-rolling-ui-only';
  ...
  function routineEvidenceJob(job,client){
    ...
    const frequency=String(client?.frequency||'').trim().toLowerCase();
    if(/ad[ -]?hoc|once[- ]?off/.test(frequency))return false;
    // Legacy/custom clients often have clean recurring diary history but no R marker.
    // Allow that history to establish the cadence; clientPattern still requires at
    // least two visits before it can generate anything.
    return true;
  }
  ...
  function rollingWorkspaceReadyV60511(){
    ...
    // Rolling maintenance is additive background work. Never compete with hydration,
    // an actual user change, an active save, or a real conflict. A previous partial
    // rolling pass can leave operationalDirty=true even when there is no remaining
    // delta; clear only that harmless stale flag so the horizon can finish.
    if(b.operationalHydrating||b.operationalSyncing||b.operationalConflict)return false;
    if(b.operationalDirty){
      try{
        const delta=typeof operationalDeltaV59394==='function'?operationalDeltaV59394():null;
        if(delta&&!delta.hasChanges&&typeof clearOperationalSaveErrorV59394==='function')clearOperationalSaveErrorV59394();
        else return false;
  ...
  let rollingBusy=false,rollingLastRun=0,rollingFollowUpPassV60517=0;
  function rollingHorizonCoverageV60517(){
    try{
      const first=mondayOf(today()),last=addDays(first,49); // eighth visible week
      ...
      for(const client of (stateNow().clients||[]).filter(c=>String(c.status||'').toLowerCase()==='active')){
        ...
        if(client.awaitingInitialRecurringPlacementV6036===true&&!(pattern.jobs||[]).length)continue;
        const target=targetForWeek(pattern,last);if(!target)continue;
        expected++;
        if(weekHasOccurrence(client.id,last)||basketHasOccurrence(client.id,last)||movedOccurrenceExistsV60511(client.id,last))covered++;
      }
      return {weekStart:last,expected,covered,missing:Math.max(0,expected-covered)};
    }catch(_){return {weekStart:'',expected:0,covered:0,missing:0};}
  }
  async function maintainRollingScheduleV60511(reason='automatic'){
    ...
    try{
      ...
      rollingLastRun=Date.now();
      const coverage=rollingHorizonCoverageV60517();
      window.__tuinbooksRollingV60511Last={reason,created:savedResult.saved.length,skippedNew,skippedNoPattern,skippedCloud,reconciled:candidates.length-savedResult.saved.length,failed:(savedResult.failed||[]).length,coverage,at:new Date().toISOString()};
      console.info('[TuinBooks v60.5.17] rolling schedule',window.__tuinbooksRollingV60511Last);
      // A partial cloud batch can finish successfully but still leave the eighth
      // visible week incomplete. Give it up to two quiet follow-up passes after the
      // new cloud revision has settled. Do not loop forever and do not interrupt a
      // real user save.
      if(coverage.missing>0&&rollingFollowUpPassV60517<2){
        rollingFollowUpPassV60517++;
        setTimeout(()=>{if(rollingWorkspaceReadyV60511())maintainRollingScheduleV60511('horizon-follow-up');},2200);
      }else if(coverage.missing===0){rollingFollowUpPassV60517=0;}
  ...
  function install() {
    ...
    // v60.5.18: rolling maintenance is owned exclusively by the approved
    // schedule-rolling-production-v6037 module. This file is UI/visit controls only.
  ...
  window.__tuinbooksSchedulePolishV60518Test={explicitClientTasksV60511,reconcileScheduleActionRenderersV60511,paintApprovedActionsV60514,hideDuplicateActionRenderersV60514,clientWorkConfirmedV60512,confirmedServiceIdsV60512,confirmedTaskLinesV60512,replaceWorkRequiredV60512};

  // v60.5.18: no rolling writer is installed here.
