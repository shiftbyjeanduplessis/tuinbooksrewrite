(() => {
  ...
  const BUILD = '60.5.7-hydration-safe-rolling-readable-cards';
  if (window.__tuinbooksVisitControlsV6057 === BUILD) return;
  window.__tuinbooksVisitControlsV6057 = BUILD;
  ...
  function managementRouteV6057(){
    try{const p=new URLSearchParams(location.search);return p.get('support')==='1'&&!!p.get('business');}catch(_){return false;}
  }
  function rollingWorkspaceReadyV6057(){
    const b=window.backendV28||{};
    const st=stateNow();
    if(!Array.isArray(st.clients)||!Array.isArray(st.schedules)||!st.clients.length)return false;
    if(managementRouteV6057()){
      return !!b.managementCoreReadyV5950 && !!(b.managementOperationalReadyV5950||b.managementOperationalReadyV59371) && !!b.lastOperationalJson;
    }
    if(String(b.mode||'').toLowerCase()==='supabase'){
      return !!b.businessId && !b.operationalHydrating && !!b.lastOperationalJson;
    }
    return true;
  }
  async function persistRollingV6057(createdIds){
    try{window.save?.();}catch(error){console.warn('[TuinBooks v60.5.7] local save warning',error);}
    const b=window.backendV28||{};
    if(String(b.mode||'').toLowerCase()!=='supabase')return true;
    try{
      if(typeof persistOperationalDocumentV59389==='function')return !!(await persistOperationalDocumentV59389());
      if(typeof syncOperationalSnapshotV41==='function')return !!(await syncOperationalSnapshotV41(true));
      return false;
    }catch(error){console.error('[TuinBooks v60.5.7] rolling cloud save failed',error);return false;}
  }
  function rollbackRollingV6057(createdIds){
    if(!createdIds?.length)return;
    const ids=new Set(createdIds.map(String));
    const st=stateNow();
    st.schedules=(st.schedules||[]).filter(job=>!ids.has(String(job.id)));
    try{window.save?.();}catch(_){ }
  }
  function movedOccurrenceExistsV6057(clientId,weekStart){
  ...
  async function maintainRollingScheduleV6057(reason='automatic'){
    if(rollingBusy)return {busy:true};
    if(!rollingWorkspaceReadyV6057())return {ready:false};
    ...
    try{
      ...
      for(const client of state.clients.filter(c=>String(c.status||'').toLowerCase()==='active')){
        ...
        for(const weekStart of weeks){
          ...
          if(weekHasOccurrence(client.id,weekStart)||basketHasOccurrence(client.id,weekStart)||movedOccurrenceExistsV6057(client.id,weekStart))continue;
          const job=makeRecurringJob(client,pattern,weekStart,target);
          state.schedules.push(job);createdIds.push(job.id);created++;
        }
      }
      if(created){
        const saved=await persistRollingV6057(createdIds);
        if(!saved){
          rollbackRollingV6057(createdIds);
          try{window.renderSchedule?.();}catch(_){ }
          window.toast?.('The rolling schedule could not be saved online. No future visits were added.','error');
          return {reason,created:0,rolledBack:created,saveFailed:true,skippedNew,skippedNoPattern};
        }
        try{window.renderSchedule?.();}catch(_){ }
        setTimeout(decorateScheduleCards,0);
      }
      rollingLastRun=Date.now();
      window.__tuinbooksRollingV6057Last={reason,created,skippedNew,skippedNoPattern,at:new Date().toISOString()};
      console.info('[TuinBooks v60.5.7] hydration-safe rolling pattern',{reason,created,skippedNew,skippedNoPattern});
      return window.__tuinbooksRollingV6057Last;
    }catch(error){
      rollbackRollingV6057(createdIds);
      console.error('[TuinBooks v60.5.7] rolling schedule failed',error);
      return {error:String(error?.message||error)};
    }finally{rollingBusy=false;}
  }
  window.maintainRollingScheduleV6057=maintainRollingScheduleV6057;
  function scheduleRollingMaintenance(reason,delay=350){
    clearTimeout(window.__tuinbooksRollingV6057Timer);
    window.__tuinbooksRollingV6057Timer=setTimeout(()=>maintainRollingScheduleV6057(reason),delay);
  }
  function waitForRollingWorkspaceV6057(reason='startup'){
    clearInterval(window.__tuinbooksRollingReadyPollV6057);
    const started=Date.now();
    const attempt=()=>{
      if(rollingWorkspaceReadyV6057()){
        clearInterval(window.__tuinbooksRollingReadyPollV6057);window.__tuinbooksRollingReadyPollV6057=null;
        scheduleRollingMaintenance(reason,120);return true;
      }
      if(Date.now()-started>90000){clearInterval(window.__tuinbooksRollingReadyPollV6057);window.__tuinbooksRollingReadyPollV6057=null;}
      return false;
    };
    if(attempt())return;
    window.__tuinbooksRollingReadyPollV6057=setInterval(attempt,500);
  }
  ...
  function install() {
    ...
    if (document.body?.dataset?.app !== 'mobile') {
      waitForRollingWorkspaceV6057('workspace-hydrated');
      document.addEventListener('click',event=>{
        if (event.target?.closest?.('.nav-tab[data-view="schedule"], [data-view="schedule"]')) waitForRollingWorkspaceV6057('schedule-open');
      },true);
      document.addEventListener('drop',event=>{
        if (event.target?.closest?.('#weeklyScheduleBoard,.schedule-day-lane')) waitForRollingWorkspaceV6057('schedule-drop');
      },true);
      setInterval(()=>{
        if(document.visibilityState==='visible'&&Date.now()-rollingLastRun>60000)waitForRollingWorkspaceV6057('visible-safety');
      },75000);
    }
  ...
  window.__tuinbooksSchedulePolishV6057Test={explicitCadence,inferCadence,clientPattern,targetForWeek,rollingWorkspaceReadyV6057,maintainRollingScheduleV6057};
