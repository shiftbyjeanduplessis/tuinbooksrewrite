/* ========================================================================== 
   ...
   ========================================================================== */
(()=>{
  const BUILD='60.4.21-flexible-onboarding-import';
  ...
  function legacyDayV60421(value){const text=keyV60420(value),days=['Monday','Tuesday','Wednesday','Thursday','Friday','Saturday'];return days.find(day=>keyV60420(day)===text)||'';}
  function legacyCurrentClientsV60421(){return typeof state!=='undefined'&&Array.isArray(state.clients)?state.clients:[];}
  function legacyCurrentTeamsV60421(){return typeof state!=='undefined'&&Array.isArray(state.teams)?state.teams:[];}
  ...
  function legacyModelV60421(parsed){
    ...
    if(schedules.length){
      ...
      schedules.forEach(row=>{
        ...
        if(!date&&!legacyDayV60421(day))issues.push(`Schedule row ${row._row}: Preferred Day must be Monday to Saturday, or provide a service date.`);
      });
    ...
    const currentName=cleanV60420(typeof state!=='undefined'?state?.business?.name:'')||cleanV60420(parsed?.fileName).replace(/[_-]+/g,' ').replace(/\b(schedule|client|clients|trial|import|tuinbooks|xlsx)\b/gi,' ').replace(/\s+/g,' ').trim()||'Current business';
  ...
  function applyLegacyTeamsV60421(model){
    ...
    model.teams.forEach((row,index)=>{
      ...
      if(!team){let id=safeImportIdV60420(rawId||name,'team');if(state.teams.some(item=>item.id===id))id=typeof uid==='function'?uid('team'):`team-${Date.now()}-${index}`;team={id,name,active:true,createdAt:now};state.teams.push(team);}
  ...
  function legacyDateForScheduleV60421(row){
    ...
    const today=typeof localDateISO==='function'?localDateISO():new Date().toISOString().slice(0,10),week=(typeof scheduleWeekStart!=='undefined'&&scheduleWeekStart)||startOfWeek(today),dates=weekDates(week),index=Math.max(0,DAYS.indexOf(day));return dates[index]||'';
  }
  function applyLegacyScheduleV60421(model,clientMap,teamMap,fileName){
    ...
    model.schedules.forEach((row,index)=>{
      ...
      if(job){const id=job.id,createdAt=job.createdAt,audit=job.audit;Object.assign(job,values,{id,createdAt:createdAt||now,audit});updated++;}else{job={id:typeof uid==='function'?uid('sch'):`sch-${Date.now()}-${index}`,createdAt:now,...values};state.schedules.push(job);created++;}
  ...
  async function commitLegacyV60421(model,fileName){
    if(model.issues.length)throw new Error('Fix the workbook issues shown before importing.');if(!workspaceWriteReadyV60420())throw new Error(workspaceWriteIssueV60420());const backup=JSON.parse(JSON.stringify(state));
    try{const teamMap=applyLegacyTeamsV60421(model),clientMap=applyLegacyClientsV60421(model,teamMap),scheduleStats=applyLegacyScheduleV60421(model,clientMap,teamMap,fileName);state.business=state.business||{};state.business.lastOnboardingImportV60421={fileName,mode:'legacy-trial',importedAt:new Date().toISOString(),clients:model.clients.length,teams:model.teams.length,schedules:model.schedules.length};if(typeof ensureV55State==='function')ensureV55State();if(typeof ensureV56State==='function')ensureV56State();save();try{typeof renderClients==='function'&&renderClients();}catch(_){}try{typeof renderSchedule==='function'&&renderSchedule();}catch(_){}return {legacy:true,teamCount:model.teams.length||teamMap.size,clientCount:model.clients.length||clientMap.size,scheduleDefinitions:model.schedules.length,...scheduleStats};}catch(error){state=backup;window.state=state;throw error;}
  }
  async function commitV60420(model,fileName){
    ...
    save();
    try{typeof renderClients==='function'&&renderClients();}catch(error){console.warn('[v60.4.21] clients render',error);}try{typeof renderSchedule==='function'&&renderSchedule();}catch(error){console.warn('[v60.4.21] schedule render',error);}try{typeof renderSettings==='function'&&renderSettings();}catch(_){ }
  ...
  async function commitFromUiV60420(){
    ...
    catch(error){console.error('[v60.4.21] onboarding import',error);toast(error?.message||'The onboarding import failed.','error');button.disabled=false;}
