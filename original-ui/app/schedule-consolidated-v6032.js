/* TuinBooks v60.3.2 PRODUCTION — Schedule authority repair
   - compact previous/current/future Schedule
   - rolling 8-week recurrence maintenance
   - day instructions + ad-hoc events on each team/day
   - day instruction first on the field route
   - recurring move scope retained
*/
(()=>{
  'use strict';
  const BUILD='60.3.2-schedule-authority-fix-production';
  ...
  async function persistAction(action){
    ...
    const payload={...(action.payload||{}),kindV6010:kind,kindV6005:kind,kindV59694:kind,createdByFeature:'schedule-consolidated-v6010'};
  ...
  function showRecurringScope(job,before){
    ...
    const changed=String(before?.date||'')!==String(job.date||'')||String(before?.teamId||'')!==String(job.teamId||'');if(!changed)return;
    const now=Date.now();if(promptedJobId===String(job.id)&&now-promptedAt<1000)return;promptedJobId=String(job.id);promptedAt=now;
    const existing=document.getElementById('recurringPlacementDialogV58930');if(existing?.open)return;
    let dialog=document.getElementById('recurringMoveDialogV6010');if(!dialog){dialog=document.createElement('dialog');dialog.id='recurringMoveDialogV6010';dialog.className='dialog recurring-placement-dialog-v58930';document.body.appendChild(dialog);}
    const client=clientFor(job)||{},team=teamFor(job.teamId)||{},name=client.name||client.displayName||'This client';
    dialog.innerHTML=`<form method="dialog" class="dialog-shell"><div class="dialog-heading"><div><span class="eyebrow">Recurring visit moved</span><h2>How should this move apply?</h2></div><button class="icon-button" value="cancel" aria-label="Close">×</button></div><p><strong>${esc(name)}</strong> was moved to <strong>${esc(dayName(job.date))}</strong>${team.name?` with <strong>${esc(team.name)}</strong>`:''}.</p><div class="recurring-scope-options-v6005"><button class="recurring-scope-card-v6005" value="once"><strong>This visit only</strong><span>Keep the normal recurring pattern unchanged.</span></button><button class="recurring-scope-card-v6005 primary" value="future"><strong>This + future visits</strong><span>Use this day/team as the recurring pattern from this visit onward.</span></button></div></form>`;
    dialog.onclose=()=>{if(dialog.returnValue==='future'){const result=applyFuturePattern(job);window.toast?.(`Recurring pattern updated to ${result.day} · ${result.team}.`);}else if(dialog.returnValue==='once'){window.toast?.('This visit only was moved. The normal recurring pattern is unchanged.');}};
    dialog.showModal();
  }
  ...
  const baseRender=(typeof window.__tuinbooksV6007RenderSchedule==='function')?window.__tuinbooksV6007RenderSchedule:((typeof window.renderScheduleConsolidatedV6007==='function')?window.renderScheduleConsolidatedV6007:window.renderSchedule);
  function renderV6010(){
    const result=baseRender?.();
    document.getElementById('v6008ScheduleOps')?.remove();document.querySelector('.v6007-ops')?.remove();document.getElementById('scheduleOperationsToolbarV6005')?.remove();document.getElementById('scheduleToolbarV6006')?.remove();
    patchCalendarButtons();injectDesktopActions();loadActions(false);decorateRollingLiveV6010();document.documentElement.dataset.scheduleRenderer='v6032-production';return result;
  }
  ...
  /* ---------- production rolling 8-week maintenance ---------- */
  function restoreCanonicalRollingV6010(){
    ...
    // These properties are live-binding setters from the core runtime bridge.
  ...
  function decorateRollingLiveV6010(){
    const host=document.getElementById('rollingScheduleOverview');
    if(!host)return;
    host.querySelectorAll('.v6005-horizon-pill').forEach(n=>n.remove());
    const head=host.querySelector('.v6001-rolling-head,.rolling-plan-head');
    if(head&&!head.querySelector('.v6010-live-horizon')){
      const pill=document.createElement('span');pill.className='v6010-live-horizon';
      pill.textContent='Rolling 8 weeks · maintained automatically';
      head.appendChild(pill);
    }
  }

  function scheduleRollingMaintenanceV6010(reason='schedule-open',delay=180){
    try{window.scheduleRollingRefreshV58929?.(reason,delay);}catch(error){console.warn('[v60.3.2] rolling refresh',error);}
  }

  function hookRecurringDialogRollingV6010(){
    const base=window.recurringPlacementDialogV58930;
    if(typeof base!=='function'||base.__v6010RollingHook)return;
    function wrapped(job,previousDate){
      const result=base(job,previousDate);
      const dialog=document.getElementById('recurringPlacementDialogV58930');
      if(dialog&&!dialog.__v6010RollingHook){
        dialog.__v6010RollingHook=true;
        dialog.addEventListener('close',()=>{
          if(dialog.returnValue==='future')scheduleRollingMaintenanceV6010('recurring-pattern-updated',120);
        });
      }
      return result;
    }
    wrapped.__v6010RollingHook=true;
    window.recurringPlacementDialogV58930=wrapped;
  }

  function applyProductionBuildMarkerV6010(){
    window.__tuinbooksBuild=BUILD;
    const marker=document.querySelector('[id^="tuinbooksBuildV"]');
    if(marker){marker.id='tuinbooksBuildV6032';marker.textContent='v60.3.2';marker.title='TuinBooks consolidated Schedule authority';}
  }

  function boot(){
    restoreCanonicalRollingV6010();
    hookRecurringDialogRollingV6010();
    applyProductionBuildMarkerV6010();
    document.documentElement.dataset.tuinbooksSchedule='v6032-production';
    ...
    if(document.body?.dataset?.app==='mobile'){
      ...
      try{renderV6010();scheduleRollingMaintenanceV6010('initial-load',260);}catch(error){console.error('[v6010] initial schedule render',error);}
      document.addEventListener('click',event=>{if(event.target?.closest?.('.nav-tab[data-view="schedule"]'))setTimeout(()=>{try{renderV6010();scheduleRollingMaintenanceV6010('schedule-open',160);}catch{}},80);},true);
      window.addEventListener('tuinbooks:runtime-ready',()=>setTimeout(()=>{try{renderV6010();loadActions(true);scheduleRollingMaintenanceV6010('workspace-ready',220);}catch{}},0));
    }
    console.info('[TuinBooks v60.3.2 production Schedule authority]',{build:BUILD,calendarActions:true,mobileInstructionFirst:true,rollingEightWeeks:true});
  }
