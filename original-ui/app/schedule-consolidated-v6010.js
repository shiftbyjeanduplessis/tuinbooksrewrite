/* TuinBooks v60.1.0 PRODUCTION — consolidated Schedule
...
(()=>{
  'use strict';
  const BUILD='60.1.0-schedule-consolidated-production';
  ...
  const baseRender=window.renderSchedule;
  function renderV6010(){
    ...
    patchCalendarButtons();injectDesktopActions();loadActions(false);decorateRollingLiveV6010();document.documentElement.dataset.scheduleRenderer='v6010-production';return result;
  }
  ...
  function scheduleRollingMaintenanceV6010(reason='schedule-open',delay=180){
    try{window.scheduleRollingRefreshV58929?.(reason,delay);}catch(error){console.warn('[v60.1.0] rolling refresh',error);}
  }
  ...
  function applyProductionBuildMarkerV6010(){
    ...
    if(marker){marker.id='tuinbooksBuildV6010';marker.textContent='v60.1.0';marker.title='TuinBooks consolidated Schedule';}
  }
  ...
  function boot(){
    ...
    document.documentElement.dataset.tuinbooksSchedule='v6010-production';
    ...
    console.info('[TuinBooks v60.1.0 production Schedule]',{build:BUILD,calendarActions:true,mobileInstructionFirst:true,rollingEightWeeks:true});
  }
