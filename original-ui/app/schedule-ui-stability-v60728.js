/* TuinBooks v60.7.28 — Schedule UI stability guard
   DOM-only, loaded last. Does not touch schedule data or persistence. */
(()=>{
  'use strict';
  const BUILD='60.7.28-management-readiness-repair';
  ...
  window.closeScheduleQueue=function closeScheduleQueueUiV60726(){
    try{closeBasketBeforeV60726?.();}catch(error){console.warn('[TuinBooks v60.7.28] basket close fallback',error);}
    closeBasketToTag();
  };
  ...
  function ensureFloatingClose(){
    let button=document.getElementById('scheduleDetailFloatingCloseV60728');
    if(button)return button;
    button=document.createElement('button');
    button.id='scheduleDetailFloatingCloseV60728';
    button.type='button';
    button.className='tb-schedule-detail-floating-close-v60728';
  ...
  function syncDetailExit(){
    ...
    document.documentElement.style.setProperty('--tb-detail-top-v60728',`${top}px`);
    button.style.top=`${top}px`;
  }

  function installStyles(){
    if(document.getElementById('scheduleUiStabilityStylesV60728'))return;
    const style=document.createElement('style');
    style.id='scheduleUiStabilityStylesV60728';
    style.textContent=`
      #scheduleDetailPanel:not(.hidden){
        top:var(--tb-detail-top-v60728,10px)!important;
        bottom:8px!important;
        height:auto!important;
        max-height:calc(100dvh - var(--tb-detail-top-v60728,10px) - 8px)!important;
      }
      .tb-schedule-detail-floating-close-v60728{
      ...
      .tb-schedule-detail-floating-close-v60728[hidden]{display:none!important}
      #tuinbooksBuildMarkerV60728{
  ...
  function installBuildMarker(){
    ...
    document.querySelectorAll('[id^="tuinbooksBuildV"],#tuinbooksBuildMarkerV60726,#tuinbooksBuildMarkerV60727,#tuinbooksBuildMarkerV60728').forEach(node=>node.remove());
    const marker=document.createElement('div');
    marker.id='tuinbooksBuildMarkerV60728';
    document.body.appendChild(marker);
    marker.textContent='BUILD v60.7.28';
    marker.title='TuinBooks production build 60.7.28';
  }
  ...
  window.__tuinbooksProductionStabilityV60728={build:BUILD,closeScheduleDetail,removeOnboardingShortcut};
