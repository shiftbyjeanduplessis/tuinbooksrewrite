window.__TUINBOOKS_MANAGEMENT_BUILD='59.6.88-stable-client-open-restore';
(() => {
  'use strict';
  ...
  function officeLoginUrl(){
    const url=new URL('https://www.tuinbooks.garden/app/client-login.html');
    if(state.currentAccountId)url.searchParams.set('business',state.currentAccountId);
    const businessName=String(state.currentAccount?.business?.name||'').trim();
  ...
  async function copyOfficeLoginLink(){
    ...
    try{
      await copyText(officeLoginUrl());
      const name=state.currentAccount?.business?.name||'this client';
      $('officeAccessMessage').textContent=`Daily login link copied for ${name}. Use this after the password has been created.`;
      toast('Daily login link copied');
  ...
  function renderOfficeAccess(){
    ...
    $('copyOfficeLoginButton').hidden=admins.length===0;
    $('copyOfficeLoginButton').textContent='Copy daily login link';
  ...
  async function adminSetupActionV59676({email,displayName,send}){
    ...
    try{
      ...
      if(send&&data.emailed===true){
        $('officeAccessMessage').textContent=`Admin access is active for ${email}. The password setup email was submitted and the same setup link is copied for WhatsApp. After setup, use Copy daily login link.`;
        toast('Admin added · email submitted · link copied');
      }else if(send){
        $('officeAccessMessage').textContent=`Admin access is active for ${email}. The email could not be submitted${data.emailError?`: ${data.emailError}`:'.'} The password setup link is copied — send it by WhatsApp. After setup, use Copy daily login link.`;
        toast('Admin added · setup link copied','error');
      }else{
        $('officeAccessMessage').textContent=`Admin access is active for ${email}. The password setup link is copied. After the password is created, use Copy daily login link.`;
