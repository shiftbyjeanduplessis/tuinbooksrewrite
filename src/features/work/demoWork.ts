import type { FieldOpportunity, WorkDay, WorkRecord } from '../../domain/types.js';
import { demoWeek } from '../schedule/demoData.js';
import { startOfWeek } from '../../domain/dates.js';
export function demoWorkDay(date:WorkDay['date']):WorkDay{
  const week=demoWeek(startOfWeek(date));const visits=week.visits.filter(v=>v.date===date);
  if(visits[0])visits[0]={...visits[0],status:'completed'};
  ...
  const opportunities:FieldOpportunity[]=visits[1]?[{id:'demo-opp-1',businessId:'demo',accountId:visits[1].accountId,scheduleJobId:visits[1].id,workRecordId:null,teamId:visits[1].teamId,category:'Irrigation',note:'Possible leak near back lawn.',photoPaths:[],status:'new',reviewDecision:'new',createdAt:new Date().toISOString(),payload:{}}]:[];
