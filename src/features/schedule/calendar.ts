...
export function renderCalendar(container:HTMLElement,data:ScheduleWeek,callbacks:CalendarCallbacks):CalendarController{
  ...
  const grid=document.createElement('div');grid.className='calendar-grid schedule-parity-board';scroll.append(grid);container.append(scroll);
  ...
  corner.innerHTML='<strong>Teams</strong><small>Route order · hours</small>';grid.append(corner);

  for(const day of days){
    const rows=data.visits.filter(v=>v.date===day&&String(v.status).toLowerCase()!=='cancelled');
    const hours=rows.reduce((sum,v)=>sum+(v.estimatedMinutes||0),0)/60;
    const safeHours=data.teams.reduce((sum,t)=>sum+safeHoursForTeam(t),0);
    const d=new Date(`${day}T12:00:00`),h=document.createElement('div');
    h.className=`day-header${day===today?' today':''}`;
    h.innerHTML=`<div class="day-heading-main"><span>${d.toLocaleDateString(undefined,{weekday:'short'})}</span><strong>${d.getDate()}</strong></div><div class="day-heading-meta"><small>${rows.length} visit${rows.length===1?'':'s'}</small><b>${hoursLabel(hours)}</b></div><div class="capacity-meter compact"><i style="width:${capacityPct(hours,safeHours)}%"></i></div>`;
    grid.append(h);
  }

  for(const team of data.teams){
    const teamWeek=data.visits.filter(v=>v.teamId===team.id&&String(v.status).toLowerCase()!=='cancelled');
    const teamHours=teamWeek.reduce((sum,v)=>sum+(v.estimatedMinutes||0),0)/60;
    const completed=teamWeek.filter(v=>String(v.status).toLowerCase()==='completed').length;
    const weeklySafe=safeHoursForTeam(team)*days.length;
    const label=document.createElement('div');label.className='team-label';
    label.innerHTML=`<div class="team-label-name"><span class="team-dot"></span><strong>${esc(team.name)}</strong></div><small>${teamWeek.length} scheduled${completed?` · ${completed} done`:''}</small><small>${hoursLabel(teamHours)} / ${hoursLabel(weeklySafe)} safe</small><div class="capacity-meter compact"><i style="width:${capacityPct(teamHours,weeklySafe)}%"></i></div>`;
    grid.append(label);
    ...
    for(const day of days){
      ...
      const usedHours=visibleRows.reduce((sum,v)=>sum+(v.estimatedMinutes||0),0)/60;
      const safeHours=safeHoursForTeam(team),completedToday=visibleRows.filter(v=>String(v.status).toLowerCase()==='completed').length;
      cell.className=`schedule-cell${usedHours>safeHours?' over-capacity':''}${day===today?' today':''}`;
      ...
      cellSummary.innerHTML=`<span><strong>${visibleRows.length}</strong> job${visibleRows.length===1?'':'s'}${completedToday?` · ${completedToday} done`:''}</span><b>${hoursLabel(usedHours)} / ${hoursLabel(safeHours)}</b><div class="capacity-meter compact"><i style="width:${capacityPct(usedHours,safeHours)}%"></i></div>`;
      cell.append(cellSummary);

      const actionBar=document.createElement('div');actionBar.className='day-action-bar';actionBar.innerHTML='<button type="button" data-new-note>+ Note</button><button type="button" data-new-event>+ Event</button>';
      ...
      actionBar.querySelector<HTMLButtonElement>('[data-new-event]')!.onclick=e=>{e.stopPropagation();callbacks.onDayAction('internal_event',day,team.id,null);};cell.append(actionBar);
      ...
      rows.forEach((visit,index)=>{
        cell.append(makeInsertButton(()=>callbacks.onAdd(day,team.id,index,sortOrderForIndex(data.visits,day,team.id,index))));
        const account=accountById.get(visit.accountId),hold=activeHold(data.clientHolds,visit.accountId);
        cell.append(makeVisitCard(visit,index+1,team,account,locationForVisit(visit,data.locations),visit.seriesId?seriesById.get(visit.seriesId):undefined,hold,beginDrag,callbacks.onResize,()=>callbacks.onVisitAction(visit,account,hold)));
      });
      cell.append(makeInsertButton(()=>callbacks.onAdd(day,team.id,rows.length,sortOrderForIndex(data.visits,day,team.id,rows.length))));grid.append(cell);
    }
  }

  let drag:{visit:Visit;ghost:HTMLElement;offsetX:number;offsetY:number;moved:boolean}|null=null;
  function beginDrag(event:PointerEvent,visit:Visit,card:HTMLElement):void{
    if(event.button!==0||(event.target as HTMLElement).closest('[data-resize-handle],[data-visit-action],[data-visit-info]')||['completed','cancelled','rescheduled','suspended'].includes(String(visit.status).toLowerCase()))return;
    event.preventDefault();const r=card.getBoundingClientRect(),ghost=document.createElement('div');ghost.className='drag-ghost';ghost.textContent=accountById.get(visit.accountId)?.name??visit.accountId;document.body.append(ghost);drag={visit,ghost,offsetX:event.clientX-r.left,offsetY:event.clientY-r.top,moved:false};place(event.clientX,event.clientY);
    const move=(e:PointerEvent)=>{if(!drag)return;e.preventDefault();drag.moved=true;place(e.clientX,e.clientY);};
    const end=(e:PointerEvent)=>{if(!drag)return;const current=drag;drag=null;current.ghost.remove();unwire();if(!current.moved)return;const el=document.elementFromPoint(e.clientX,e.clientY);if(el?.closest('[data-basket-drop]')){void callbacks.onQueue(current.visit.id);return;}const target=el?.closest<HTMLElement>('[data-schedule-cell]');const date=target?.dataset.date,teamId=target?.dataset.teamId;if(date&&teamId)void callbacks.onMove({visitId:current.visit.id,date:date as IsoDate,teamId,sortOrder:nextSortOrder(data.visits,date,teamId)});};
...
function makeInsertButton(onClick:()=>void):HTMLElement{
  const wrap=document.createElement('div');wrap.className='insert-zone';const btn=document.createElement('button');btn.type='button';btn.className='additional-trigger';btn.title='Add an additional visit here';btn.innerHTML='<b>+</b><span>Additional visit</span>';btn.onclick=e=>{e.stopPropagation();onClick();};wrap.append(btn);return wrap;
}

function makeVisitCard(visit:Visit,route:number,team:Team,account:Account|undefined,location:ServiceLocation|null,series:ScheduleSeries|undefined,hold:ClientServiceHold|null,begin:(e:PointerEvent,v:Visit,c:HTMLElement)=>void,onResize:(input:ResizeVisitInput)=>Promise<void>,onAction:()=>void):HTMLElement{
  ...
  card.className=`visit-card visit-${visit.visitType} status-${status}${visit.recurrenceManualOverride?' recurrence-exception':''}${hold?' do-not-service':''}`;card.dataset.visitId=visit.id;
  card.style.setProperty('--visit-height',`${Math.max(74,Math.min(172,74+Math.max(0,visit.estimatedMinutes-60)/15*4))}px`);
  ...
  card.innerHTML=`<div class="visit-card-topline"><span class="visit-route" title="Route order">${route}</span><strong>${esc(account?.name??visit.accountId)}</strong><span class="visit-badges">${badges}<button type="button" data-visit-info class="visit-info-button" aria-label="Visit information" title="Visit information">i</button><button type="button" data-visit-action class="visit-action-button" aria-label="Visit actions" title="Visit actions">•••</button></span></div><div class="visit-location">${esc(location?.address||'Address not linked')}</div><div class="visit-suburb">${esc(location?.suburb||'')}</div>${taskLine}<div class="visit-card-footer"><span>${esc(type)}</span><b>${duration}</b></div>${locked?'':'<span class="resize-handle" data-resize-handle title="Drag to change duration"></span>'}`;
  card.addEventListener('pointerdown',e=>begin(e,visit,card));
  ...
  if(handle)handle.addEventListener('pointerdown',event=>{event.preventDefault();event.stopPropagation();const startY=event.clientY,start=visit.estimatedMinutes||60;let current=start;const move=(e:PointerEvent)=>{e.preventDefault();current=normaliseDuration(start+Math.round((e.clientY-startY)/10)*15);card.querySelector<HTMLElement>('.visit-card-footer b')!.textContent=`${current} min`;card.style.setProperty('--visit-height',`${Math.max(74,Math.min(172,74+Math.max(0,current-60)/15*4))}px`);};const end=()=>{off();if(current!==visit.estimatedMinutes)void onResize({visitId:visit.id,estimatedMinutes:current});};const off=()=>{window.removeEventListener('pointermove',move);window.removeEventListener('pointerup',end);};window.addEventListener('pointermove',move,{passive:false});window.addEventListener('pointerup',end,{once:true});});
  return card;
}
...
function visitTypeLabel(visit:Visit):string{
  if(visit.visitType==='additional')return 'Additional visit';if(visit.visitType==='quoted')return 'Quoted work';if(visit.visitType==='once-off')return 'Once-off';return 'Recurring';
}
function statusLabel(status:string):{key:string;label:string}{
  if(status==='completed')return{key:'completed',label:'Completed'};if(status==='missed')return{key:'attention',label:'Missed'};if(status==='cancelled')return{key:'cancelled',label:'Cancelled'};if(status==='rescheduled')return{key:'neutral',label:'Rescheduled'};if(status==='suspended')return{key:'neutral',label:'Suspended'};if(status==='deferred')return{key:'attention',label:'Deferred'};return{key:'scheduled',label:'Scheduled'};
}
function taskSummary(payload:Record<string,unknown>):string{
  const list=payload.visitTasks;if(Array.isArray(list)){const text=list.filter(v=>typeof v==='string'&&v.trim()).map(String).join(' · ');if(text)return text;}
  for(const key of ['customTasks','serviceDescription','description','reason','task']){const value=payload[key];if(typeof value==='string'&&value.trim())return value.trim();}
  return '';
}
...
function safeHoursForTeam(team:Team):number{return Math.max(.25,Number(team.capacityHours||8)-Math.max(0,Number(team.bufferHours||0)));}
function hoursLabel(hours:number):string{return `${Math.round(hours*10)/10}h`;}
function capacityPct(used:number,capacity:number):number{return Math.max(0,Math.min(100,capacity?used/capacity*100:0));}
