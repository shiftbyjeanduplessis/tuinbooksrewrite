...
type WorkItem={label:string;icon:string;kind:'service'|'task'};
...
export function openVisitActionDialog(
  ...
  d.querySelectorAll<HTMLButtonElement>('[data-op]').forEach(button=>button.onclick=()=>{
    ...
    if(op==='service-normal'){
      ...
      if(visitDns.active)operations.push(callbacks.onVisitDoNotService(visit.id,false,visitDns.reason,visitDns.note));
      if(hold)operations.push(callbacks.onClientHold(visit.accountId,false,hold.reason||'Do not service',hold.note||''));
      if(!operations.length){close();return;}
...
function workItemsForVisitDetail(visit:Visit,agreement:ServiceAgreement|undefined,services:BusinessService[]):WorkItem[]{
  const rows:WorkItem[]=[];
  const seen=new Set<string>();
  const add=(label:unknown,kind:'service'|'task',icon?:string)=>{const text=String(label??'').trim();const key=text.toLowerCase();if(!text||seen.has(key))return;seen.add(key);rows.push({label:text,kind,icon:icon||serviceIcon(text,text)});};
  const serviceIds=visit.serviceIds.length?visit.serviceIds:(agreement?.serviceIds??[]);
  for(const id of serviceIds){const service=services.find(row=>row.id===id);add(service?.name||id,'service',serviceIcon(id,service?.name||id));}
  const payload=visit.payload??{};
  const addTaskValue=(value:unknown)=>{if(Array.isArray(value))value.forEach(v=>add(v,'task','task'));else if(typeof value==='string')value.split(/\r?\n|\s*;\s*/).forEach(v=>add(v,'task','task'));};
  ...
  if(serviceDescription&&!rows.some(row=>row.label.toLowerCase()===serviceDescription.toLowerCase()))addTaskValue(serviceDescription);
  if(!rows.length)add('Garden service','service','maintenance');
  return rows;
}

function serviceIcon(id:string,label:string):string{
  const text=`${id} ${label}`.toLowerCase();
  if(/irrig.*repair|repair.*irrig/.test(text))return'repair';
  if(/irrig|sprinkler|water/.test(text))return'irrigation';
  if(/hedge/.test(text))return'hedge';
  if(/prun/.test(text))return'pruning';
  if(/tree/.test(text))return'tree';
  if(/edg/.test(text))return'edging';
  if(/lawn|grass|mow/.test(text))return'lawn';
  if(/treat|fertili|feed|spray/.test(text))return'treatment';
  if(/waste|refuse|remove/.test(text))return'waste';
  if(/clean.?up|season/.test(text))return'cleanup';
  if(/landscap|install/.test(text))return'landscaping';
  if(/other/.test(text))return'other';
  return'maintenance';
}

function workIconSvg(name:string):string{
