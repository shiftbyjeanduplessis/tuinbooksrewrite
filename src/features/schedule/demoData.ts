import type { ScheduleSeries, ScheduleWeek, Visit } from '../../domain/types.js';
import { addDays, startOfWeek, todayIso } from '../../domain/dates.js';

export function demoWeek(requestedWeekStart?: ScheduleWeek['weekStart']): ScheduleWeek {
  ...
  const visits:Visit[]=[...mapped,...accounts.slice(3,14).map((account,i)=>({id:`visit-${i+4}`,businessId:'demo',date:addDays(weekStart,(i+1)%5),accountId:account.id,serviceLocationId:`site-${i+4}`,teamId:teams[i%teams.length].id,status:'scheduled',estimatedMinutes:i===1?0:60+(i%3)*15,sortOrder:200+(Math.floor(i/3)*100),serviceIds:[],visitType:i===1?'additional' as const:'routine' as const,billingDisposition:i===1?'additional' as const:'routine' as const,seriesId:null,seriesSlotId:null,occurrenceDate:null,recurrenceManualOverride:false,payload:i===1?{visitType:'additional',workKind:'additional-visit'}:{workMarker:'R'},updatedAt:null}))];
  const queueItems=accounts.slice(15,19).map((account,i)=>({id:`queue-demo-${i+1}`,businessId:'demo',accountId:account.id,serviceLocationId:`site-${15+i+1}`,sourceVisitId:null,seriesId:null,seriesSlotId:null,occurrenceDate:null,originalDate:null,originalTeamId:null,estimatedMinutes:45+(i*15),serviceIds:[],visitType:i===2?'quoted' as const:'routine' as const,billingDisposition:i===2?'quoted' as const:'routine' as const,reason:i===2?'Accepted quoted work':'Unscheduled routine visit',payload:{}}));
  ...
  const dayActions=[{id:'action-demo-note',businessId:'demo',date:addDays(weekStart,1),teamId:'team-1',kind:'team_note' as const,title:'Day instruction',detail:'Collect green bags before first visit.',time:'',status:'active' as const,response:'',resolvedAt:null,updatedAt:null},{id:'action-demo-event',businessId:'demo',date:addDays(weekStart,3),teamId:'team-2',kind:'internal_event' as const,title:'Vehicle service',detail:'Workshop stop',time:'11:30',status:'active' as const,response:'',resolvedAt:null,updatedAt:null}];
  return {weekStart,weekEnd:addDays(weekStart,6),teams,accounts,locations,visits,queueItems,series,clientHolds,dayActions};
}
function recurringVisit(id:string,accountId:string,siteId:string,teamId:string,date:Visit['date'],sortOrder:number,minutes:number,seriesId:string,seriesSlotId:string):Visit{return{id,businessId:'demo',date,accountId,serviceLocationId:siteId,teamId,status:'scheduled',estimatedMinutes:minutes,sortOrder,serviceIds:[],visitType:'routine',billingDisposition:'routine',seriesId,seriesSlotId,occurrenceDate:date,recurrenceManualOverride:false,payload:{workMarker:'R',autoGenerated:true,scheduleSeriesId:seriesId},updatedAt:null};}
