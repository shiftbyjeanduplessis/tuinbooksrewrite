...
export function renderSchedulePage(root:HTMLElement,identity:ScheduleIdentity,navigation:WorkspaceNavigation):void{
  ...
  const selectionCount=root.querySelector<HTMLElement>('#dragSelectionCount')!;
  ...
  async function moveVisitGroup(moves:MoveVisitInput[]){
    ...
    try{
      ...
      for(const move of moves)visits=moveVisitOptimistically(visits,{...move,scope:'one'});
  ...
  async function queueVisitGroup(visitIds:string[]){
    if(!data||!visitIds.length)return;
  ...
  function openVisitActions(visit:Visit,account:any,hold:ClientServiceHold|null){if(!data)return;openVisitActionDialog(visit,account,data.teams,hold,{onCancel:cancelVisit,onUndoCancel:undoCancel,onMissed:markMissed,onResolveMissed:resolveMissed,onReschedule:rescheduleMissed,onSuspend:setSuspension,onVisitDoNotService:setVisitDoNotService,onClientHold:setClientHold});}
