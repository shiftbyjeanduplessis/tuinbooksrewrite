import type { AdditionalVisitInput, MoveVisitInput, ResizeVisitInput, ScheduleQueueItem, ServiceLocation, Visit } from './types.js';
...
export function serviceLocationIdFromLegacyPayload(payload: Record<string, unknown>): string | null {
  return stringField(payload, 'serviceLocationId', 'serviceSiteId', 'siteId', 'service_site_id');
}
export function visitTypeFromPayload(payload: Record<string, unknown>): Visit['visitType'] {
  const raw = (stringField(payload, 'visitType', 'workKind', 'workType', 'type') || '').toLowerCase();
  if (raw.includes('additional') || raw === 'a') return 'additional';
  if (raw.includes('quote') || raw.includes('extra')) return 'quoted';
  if (raw.includes('once')) return 'once-off';
  return 'routine';
}
export function billingDispositionFromPayload(payload: Record<string, unknown>): Visit['billingDisposition'] {
  const raw = (stringField(payload, 'billingDisposition', 'cancellationBilling', 'billing') || '').toLowerCase();
  if (raw.includes('no') && raw.includes('charge')) return 'no-charge';
  if (raw.includes('charge')) return 'charge';
  if (raw.includes('additional')) return 'additional';
  if (raw.includes('quote')) return 'quoted';
  return visitTypeFromPayload(payload)==='additional'?'additional':'routine';
}
export function moveVisitOptimistically(visits: Visit[], move: MoveVisitInput): Visit[] {
  return visits.map(visit => {
    if(visit.id!==move.visitId)return visit;
...
export function resizeVisitOptimistically(visits: Visit[], input: ResizeVisitInput): Visit[] {
...
export function visitsForCell(visits: Visit[], date: string, teamId: string): Visit[] {
  return visits.filter(v => v.date === date && v.teamId === teamId).sort((a, b) => a.sortOrder - b.sortOrder || a.id.localeCompare(b.id));
}
export function locationForVisit(visit: Visit, locations: ServiceLocation[]): ServiceLocation | null {
  if (visit.serviceLocationId) { const exact = locations.find(location => location.id === visit.serviceLocationId); if (exact) return exact; }
...
export function nextSortOrder(visits: Visit[], date: string, teamId: string): number {
  const inCell = visitsForCell(visits, date, teamId); return inCell.length ? Math.max(...inCell.map(v => v.sortOrder)) + 100 : 100;
}
export function sortOrderForIndex(visits: Visit[], date: string, teamId: string, index: number): number {
  const rows=visitsForCell(visits,date,teamId); const at=Math.max(0,Math.min(rows.length,index));
  const before=rows[at-1]?.sortOrder; const after=rows[at]?.sortOrder;
  if(before===undefined&&after===undefined)return 100;
  if(before===undefined)return after! - 100;
  if(after===undefined)return before + 100;
  if(after-before>1)return Math.floor((before+after)/2);
  return before+1;
}
export function normaliseDuration(minutes:number):number { return Math.max(15,Math.min(480,Math.round(Number(minutes||0)/15)*15 || 15)); }
export function queueVisitOptimistically(visits:Visit[],queueItems:ScheduleQueueItem[],visitId:string):{visits:Visit[];queueItems:ScheduleQueueItem[]} {
  const visit=visits.find(row=>row.id===visitId); if(!visit)return {visits,queueItems};
  const item:ScheduleQueueItem={id:`queue-${visit.id}`,businessId:visit.businessId,accountId:visit.accountId,serviceLocationId:visit.serviceLocationId,sourceVisitId:visit.id,seriesId:visit.seriesId,seriesSlotId:visit.seriesSlotId,occurrenceDate:visit.occurrenceDate,originalDate:visit.date,originalTeamId:visit.teamId,estimatedMinutes:visit.estimatedMinutes,serviceIds:[...visit.serviceIds],visitType:visit.visitType,billingDisposition:visit.billingDisposition,reason:'Moved from calendar',payload:{...visit.payload}};
  return {visits:visits.filter(row=>row.id!==visitId),queueItems:[...queueItems.filter(row=>row.sourceVisitId!==visit.id),item]};
}
export function placeQueueItemOptimistically(visits:Visit[],queueItems:ScheduleQueueItem[],queueItemId:string,date:Visit['date'],teamId:string,sortOrder:number):{visits:Visit[];queueItems:ScheduleQueueItem[]} {
  const item=queueItems.find(row=>row.id===queueItemId); if(!item)return {visits,queueItems};
  const visit:Visit={id:item.sourceVisitId||`visit-${item.id}`,businessId:item.businessId,date,accountId:item.accountId,serviceLocationId:item.serviceLocationId,teamId,status:'scheduled',estimatedMinutes:item.estimatedMinutes,sortOrder,serviceIds:[...item.serviceIds],visitType:item.visitType,billingDisposition:item.billingDisposition,seriesId:item.seriesId,seriesSlotId:item.seriesSlotId,occurrenceDate:item.occurrenceDate,recurrenceManualOverride:!!item.seriesId,payload:{...item.payload,v2RestoredFromQueue:true},updatedAt:null};
  return {visits:[...visits,visit],queueItems:queueItems.filter(row=>row.id!==queueItemId)};
}
export function buildAdditionalVisit(input:AdditionalVisitInput,businessId:string):Visit {
