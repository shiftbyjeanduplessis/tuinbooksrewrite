...
export interface DayActionInput{id:string;date:IsoDate;teamId:string;kind:'team_note'|'internal_event';title:string;detail:string;time:string;}
...
export function upsertDayActionOptimistically(actions:ScheduleDayAction[],businessId:string,input:DayActionInput):ScheduleDayAction[]{
  const row:ScheduleDayAction={id:input.id,businessId,date:input.date,teamId:input.teamId,kind:input.kind,title:input.kind==='team_note'?'Day instruction':input.title.trim(),detail:input.detail.trim(),time:input.kind==='team_note'?'':input.time,status:'active',updatedAt:null};return [...actions.filter(a=>a.id!==input.id),row];
}
